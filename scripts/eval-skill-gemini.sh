#!/usr/bin/env bash
# Fulcrum eval-skill-gemini — measure a skill's trigger rate via the Gemini CLI.
#
# Usage:
#   scripts/eval-skill-gemini.sh <skill-name> [--queries FILE]
#                                              [--model NAME]
#                                              [--runs-per-query N]
#                                              [--results-dir DIR]
#                                              [--match-words "w1,w2,..."]
...
#
# Gemini skill install: fulcrum skills sync puts skills at
#   ~/.gemini/extensions/fulcrum-skills/skills/<name>/SKILL.md
# Link extension once: gemini extensions link ~/.gemini/extensions/fulcrum-skills
...
command -v gemini >/dev/null 2>&1 || {
  echo "fulcrum: \`gemini\` CLI not on PATH. Install Gemini CLI." >&2
  exit 1
}
...
[ -z "$RESULTS_DIR" ] && RESULTS_DIR="$(mktemp -d -t "fulcrum-gemini-eval-$SKILL-XXXXXX")"
mkdir -p "$RESULTS_DIR"
...
{
  echo "fulcrum gemini eval — $SKILL"
  echo "  queries   : $QUERIES"
  echo "  model     : ${MODEL:-gemini default}"
...
# Confirm skill is installed under Gemini extension path.
SKILL_INSTALLED=""
for cand in \
  "$HOME/.gemini/extensions/fulcrum-skills/skills/$SKILL/SKILL.md" \
  "$HOME/.gemini/extensions/fulcrum-skills/$SKILL/SKILL.md"; do
  [ -f "$cand" ] && { SKILL_INSTALLED="$cand"; break; }
done
if [ -z "$SKILL_INSTALLED" ]; then
  cat >&2 <<EOF
fulcrum: skill '$SKILL' not found under ~/.gemini/extensions/fulcrum-skills/.
Run: fulcrum skills sync
Then: gemini extensions link ~/.gemini/extensions/fulcrum-skills
Without this the agent cannot discover the skill.
EOF
  exit 1
fi
...
# gemini --output-format json emits JSON on stdout; debug/extension load on stderr.
detect_trigger() {
  local combined="$1"
  local lc
...
while IFS= read -r entry; do
  ...
  for run in $(seq 1 "$RUNS"); do
    ...
    STDOUT_FILE=$(mktemp)
    STDERR_FILE=$(mktemp)
    gemini "${GEMINI_ARGS[@]}" </dev/null >"$STDOUT_FILE" 2>"$STDERR_FILE" || true
    cat "$STDERR_FILE" >> "$LOG"
