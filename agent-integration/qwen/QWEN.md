# Fulcrum Agent OS — Qwen Code Integration

This file is auto-loaded by Qwen Code. It configures your connection to the Fulcrum agent control plane.

## Canonical rules (modular via `@./imports`)

Qwen Code supports file-includes in context files per Qwen Code context imports
...
Fulcrum is CLI-first. Use `fulcrum action exec <action>` as the standard path in skills and automation; use the `fulcrum` MCP server when Qwen Code needs an MCP tool surface. It runs as a local stdio process via `fulcrum serve mcp`, and its exposed tool set can be filtered per runtime or agent.

Recommended Qwen MCP command when the PreToolUse / PostToolUse hooks are installed:
...
In Qwen Code, MCP tool names use underscores. All tools are prefixed `mcp_fulcrum_`.
...
project_id:   string (optional)
model:        string (optional) — e.g., "qwen-coder"
...
If Qwen Code is invoking Fulcrum through MCP-native tools instead of shell commands, use the equivalent `mcp_fulcrum_*` tools from the catalog above.
...
The hook runs `fulcrum hook qwen` on every tool call and reads the tool event from stdin.
