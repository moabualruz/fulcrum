...
- `bun test src/server/trpc/routers/__tests__ tests/trpc/router.test.ts src/architecture/boundary.test.ts` passed router tests but failed the shared boundary allowlist because concurrent Plan 23 files (`src/server/trpc/routers/automations.ts`, `custom-fields.ts`, `orgs.ts`, `workflows.ts`) were already clean while the committed expected-debt list still contained them. Plan 28-owned routers were absent from the residual direct-access and raw-SQL results, so the unrelated allowlist drift was not fixed here.
