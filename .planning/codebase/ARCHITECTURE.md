...
│ `apps/web/`      │ `apps/cli/src/` │ `apps/tui/src/index.ts`    │
│ `apps/web/src/`  │ `main.ts`       │ `apps/tui/src/screens/`    │
│ hooks.server.ts` │                  │                       │
└────────┬─────────┴────────┬─────────┴──────────┬────────────┘
         │                  │                     │
         ▼                  ▼                     ▼
...
│  `apps/server/src/trpc/router.ts`  `apps/server/src/runtime/trpc/routers/`           │
│  `apps/server/src/trpc/routers/`   `src/services/`                      │
├─────────────────────────────────────────────────────────────┤
│                Public REST / OpenAPI Boundary                │
│  `apps/server/src/api/hono.ts`  `apps/server/src/api/routes/`                       │
└────────┬──────────────────────┬─────────────────────────────┘
         │                      │
         ▼                      ▼
┌───────────────────────┐ ┌───────────────────────────────────┐
...
│ `src/db/db.module.ts` │ │ `apps/web/src/lib/server/db.ts`     │
└───────────┬───────────┘ └────────────────┬──────────────────┘
            │                              │
            ▼                              ▼
┌─────────────────────────────────────────────────────────────┐
...
|-----------|----------------|------|
| Root CLI dispatcher | Parses top-level `fulcrum` command and lazy-loads subcommands | `apps/cli/src/main.ts` |
| CLI command hub | Handles broad product commands, DB bootstrap, web server launch, TUI launch | `apps/cli/src/index.ts` |
| Web request hook | Initializes ProductDb singleton, mounts Better-Auth and tRPC, forks ORM request context | `apps/web/src/hooks.server.ts` |
| Web route loaders | Load server data for SvelteKit pages and API route handlers | `apps/web/src/routes/` |
| TUI app root | Renders keyboard-first terminal screens and consumes in-process tRPC caller | `apps/tui/src/index.ts` |
| TUI router | Maintains terminal route state and back stack | `apps/tui/src/router.ts` |
| tRPC root router | Composes internal application API across all domains | `apps/server/src/trpc/router.ts` |
| tRPC context | Carries session, org, request-scoped `EntityManager`, DI container, deprecated ProductDb shim | `apps/server/src/trpc/context.ts` |
| tRPC base procedures | Adds request IDs and OpenTelemetry span wrapper to procedures | `apps/server/src/trpc/trpc.ts` |
| Domain tRPC routers | Implement procedure handlers for tasks, docs, auth, flags, inference, reports, etc. | `apps/server/src/runtime/trpc/routers/`, `apps/server/src/trpc/routers/` |
| Public REST API | Builds Hono OpenAPI app gated by `public-api` feature flag | `apps/server/src/api/hono.ts` |
| REST route adapters | Register public API routes backed by ProductDb/services or static OpenAPI metadata | `apps/server/src/api/routes/` |
...
| ProductDb shim | Adapts ORM connection to legacy `ProductDb.query/exec/close` interface for web compatibility | `apps/web/src/lib/server/db.ts` |
...
| Collaboration | Provides Yjs document collaboration server/client surfaces | `src/collab/`, `apps/server/src/runtime/yjs-server.ts`, `apps/web/src/lib/collab/` |
...
**Key Characteristics:**
- Use `apps/server/src/trpc/router.ts` as shared internal API for Web, CLI, TUI, and tests.
- Use `apps/server/src/api/hono.ts` for external REST/OpenAPI exposure; keep feature gate behavior in `apps/server/src/api/feature-flags.ts`.
- Use request-scoped MikroORM `EntityManager` plus `needle-di` container for canonical data access.
- Keep `ProductDb` compatibility isolated to `src/product-kernel/`, `apps/web/src/lib/server/db.ts`, and explicit deprecated context fields.
...
- Location: `apps/web/`, `apps/cli/src/`, `apps/tui/src/`.
...
- Location: `apps/server/src/trpc/`, `apps/server/src/runtime/trpc/routers/`, `apps/server/src/api/`.
...
- Location: `src/product-kernel/`, `apps/web/src/lib/server/db.ts`.
...
1. Request enters SvelteKit server hook (`apps/web/src/hooks.server.ts`).
2. Hook initializes ProductDb singleton once with `initProductDb()` (`apps/web/src/lib/server/db.ts`).
3. Hook lazily initializes MikroORM, Better-Auth, `FlagRegistry`, and request-scoped `Container` (`apps/web/src/hooks.server.ts`).
4. `/api/auth/**` delegates to `AuthService.handler` (`src/auth/index.ts`).
5. `/api/trpc/**` delegates to `fetchRequestHandler` with `appRouter` and `createContext()` (`apps/web/src/hooks.server.ts`, `apps/server/src/trpc/router.ts`, `apps/server/src/trpc/context.ts`).
6. Procedure invokes service/repository logic in `apps/server/src/runtime/trpc/routers/`, `apps/server/src/trpc/routers/`, or `src/services/`.
...
1. SvelteKit route loader runs from `apps/web/src/routes/**/+page.server.ts`.
2. Loader reads `event.locals` populated by `apps/web/src/hooks.server.ts`.
...
5. Client-only shared state belongs under `apps/web/src/lib/state/` or route-local component state.

### CLI Command Path

1. `fulcrum` starts at `apps/cli/src/main.ts`.
2. Top-level switch lazy-loads command-specific modules or delegates broad commands to `apps/cli/src/index.ts`.
3. DB-backed CLI commands build a `needle-di` container and initialize MikroORM/PGlite or Postgres (`apps/cli/src/index.ts`).
...
1. `fulcrum tui` dispatches through `apps/cli/src/main.ts` and `apps/cli/src/index.ts`.
2. TUI app starts in `apps/tui/src/index.ts`.
3. `TuiCaller` invokes in-process tRPC-like methods; tests inject FakeTTY and fake callers.
4. `TuiRouter` maps terminal paths to screens (`apps/tui/src/router.ts`, `apps/tui/src/screens/`).
5. Screens load data on mount and render with the OpenTUI/FakeTTY-compatible renderer.

### Public REST API Path

1. Hono app is created by `createPublicApiRouter()` (`apps/server/src/api/hono.ts`).
2. `/api/openapi.json` returns static generated OpenAPI document even outside feature-gated `/api/v1`.
3. `/api/v1/*` requests are blocked with 404 unless `isPublicApiEnabled()` passes (`apps/server/src/api/feature-flags.ts`).
4. When deps are provided, Hono context receives `ProductDb` and optional tRPC object (`apps/server/src/api/hono.ts`).
5. Route adapters in `apps/server/src/api/routes/` call kernel/service implementations and return JSON.

### Orchestration Flow

1. Agent run rows are created in `agent_runs` via routers/services using `src/db/entities/orchestration/AgentRun.ts`.
2. Symphony orchestrator starts from web server launch or command path (`apps/cli/src/index.ts`, `src/orchestration/symphony/orchestrator.ts`).
...
5. Document collaboration uses Yjs server/client paths (`apps/server/src/runtime/yjs-server.ts`, `src/collab/server.ts`, `apps/web/src/lib/collab/`).

**State Management:**
- Server request state: `event.locals` in `apps/web/src/hooks.server.ts`, `TrpcContext` in `apps/server/src/trpc/context.ts`.
...
- Web client state: Svelte stores under `apps/web/src/lib/state/` and page/component local state.
- TUI state: screen-local state in `apps/tui/src/screens/` plus `TuiRouter` history.

## Key Abstractions

**`TrpcContext`:**
- Purpose: Request/session/org/data-access envelope for internal API calls.
- Examples: `apps/server/src/trpc/context.ts`, `apps/web/src/hooks.server.ts`, `apps/web/src/routes/api/trpc/[...path]/+server.ts`.
...
- Examples: `src/db/db.module.ts`, `apps/web/src/hooks.server.ts`, `apps/cli/src/index.ts`.
...
- Examples: `src/product-kernel/db/types.ts`, `apps/web/src/lib/server/db.ts`, `apps/server/src/trpc/context.ts`.
...
- Examples: `src/flags/registry.ts`, `src/db/db.module.ts`, `apps/server/src/api/feature-flags.ts`.
...
**Web tRPC Route:**
- Location: `apps/web/src/routes/api/trpc/[...path]/+server.ts`.
...
- Location: `apps/web/src/routes/api/v1/+server.ts`.
- Triggers: SvelteKit API requests to `/api/v1`.
- Responsibilities: feature-gated public API placeholder/static handler; Hono canonical app lives in `apps/server/src/api/hono.ts`.

**Hono Public API Factory:**
- Location: `apps/server/src/api/hono.ts`.
...
**TUI Runtime:**
- Location: `apps/tui/src/index.ts`.
...
- **Global state:** `apps/web/src/lib/server/db.ts` holds ProductDb singleton; `apps/web/src/hooks.server.ts` holds lazy `_runtimePromise`; `src/orchestration/symphony/orchestrator.ts` holds process-local `claimQueues`; subscription/event modules expose process-local buses; these must be reset explicitly in tests.
- **Circular imports:** Web hooks use lazy imports for `apps/server/src/trpc/router.ts`, `apps/server/src/trpc/context.ts`, `src/auth/index.ts`, and DB setup to avoid Vite SSR and entity graph import cycles.
...
**Do this instead:** Add/extend entities and repositories under `src/db/`, bind them in `src/db/db.module.ts`, and expose behavior through services such as `src/services/TaskService.ts` or routers in `apps/server/src/runtime/trpc/routers/`.
...
**Do this instead:** Put rules in `src/services/` or a domain folder and call them from `apps/server/src/runtime/trpc/routers/`, CLI local caller paths, and TUI caller paths.

### Importing Web From Core Layers

**What happens:** Modules under `src/product-kernel/`, `apps/cli/src/`, or `src/services/` import `apps/web/`.
**Why it's wrong:** It breaks the enforced dependency direction and can pull SvelteKit/Vite-only code into CLI or tests.
**Do this instead:** Move shared logic to `src/services/`, `src/utils/`, `src/platform/`, or domain folders; keep Svelte UI adapters under `apps/web/`.
...
**Do this instead:** Follow lazy import pattern in `apps/web/src/hooks.server.ts` for `apps/server/src/trpc/router.ts`, `apps/server/src/trpc/context.ts`, `src/auth/index.ts`, and DB modules.

### Duplicate Public API Gating

**What happens:** A route defines its own public API feature flag parsing.
**Why it's wrong:** `apps/server/src/api/feature-flags.ts` is the canonical `public-api` gate; duplicate gates can expose routes inconsistently.
**Do this instead:** Import `isPublicApiEnabled()` from `apps/server/src/api/feature-flags.ts` and keep public REST route registration in `apps/server/src/api/hono.ts` / `apps/server/src/api/routes/`.
...
**Patterns:**
- tRPC error formatter injects request ID in `apps/server/src/trpc/trpc.ts`.
...
- Web auth/session hydration catches auth/runtime failures and falls back to empty session where appropriate in `apps/web/src/hooks.server.ts`.
- Best-effort side effects catch non-critical failures, e.g. watcher subscription in `src/services/TaskService.ts`.
- Public REST feature gate returns 404 rather than route details when disabled in `apps/server/src/api/hono.ts`.

## Cross-Cutting Concerns

**Logging:** Use console output for CLI/web startup and selected server failures (`apps/cli/src/index.ts`, `apps/web/src/hooks.server.ts`); use event/audit rows for durable domain history (`src/db/entities/core/Event.ts`, `src/router/telemetry.ts`, `src/events/`).

**Validation:** Use Zod in routers/schemas (`apps/server/src/trpc/router.ts`, `src/router/decision-schema.ts`, `apps/server/src/api/routes/`) and service-level validation for workflow/custom field constraints (`src/services/WorkflowService.ts`, `src/services/FieldDependencyService.ts`).

**Authentication:** Better-Auth handles web session APIs through `src/auth/index.ts` and `apps/web/src/hooks.server.ts`; public REST uses bearer API-key auth in `apps/server/src/api/auth.ts`; local dev can auto-create a session unless `FULCRUM_REQUIRE_AUTH` is set.

**Authorization:** tRPC protected procedure metadata and permission middleware live under `apps/server/src/trpc/` and `apps/server/src/runtime/trpc/middleware/`; project state records `EntityManager` as canonical data access and bearer API-key as unified REST auth.

**Feature Flags:** Register and evaluate flags through `src/flags/registry.ts`, `src/db/db.module.ts`, and specific helpers such as `apps/server/src/api/feature-flags.ts`.

**Migrations:** CLI DB commands use `apps/cli/src/index.ts` and `src/db/migrator-service.ts`; ProductDb compatibility still has SQL migration runner in `src/product-kernel/db/migrate.ts`.
