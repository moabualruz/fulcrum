...
- Vitest 4.1.5 for selected web component/unit coverage under `apps/web/tests/vitest/**/*.test.ts`.
- Playwright 1.59.1 for web browser smoke, accessibility, and full e2e tests under `apps/web/tests/`.
- Config: root `bunfig.toml`, root `scripts/test-root.ts`, web `apps/web/vitest.config.ts`, web `apps/web/playwright.config.ts`.

**Assertion Library:**
- Bun assertions via `expect` from `bun:test`: `src/services/TaskService.test.ts`, `tests/db/migrator-service.test.ts`.
- Vitest assertions for web vitest suites configured by `apps/web/vitest.config.ts`.
- Playwright assertions via `expect` from `@playwright/test`: `apps/web/tests/e2e/phase08-surface-delivery.spec.ts`.

**Run Commands:**
```bash
bun run ci                                # Full local CI gate
bun test                                  # Raw Bun test runner
bun run scripts/test-root.ts              # Root test discovery excluding apps/web and tests/a11y
bun run scripts/test-root.ts --coverage   # Root coverage gate
(cd apps/web && bun run check)             # SvelteKit + svelte-check
(cd apps/web && bun run test)              # Bun Svelte lib tests
(cd apps/web && bun run web:test)          # Vitest web tests
(cd apps/web && bun run web:test -- --coverage) # Vitest coverage
(cd apps/web && bun run web:a11y)          # Playwright accessibility subset
(cd apps/web && bun run web:e2e:smoke)     # Playwright smoke
...
**Location:**
- Co-located root unit tests live beside implementation: `src/router/rules-engine.test.ts`, `src/services/TaskService.test.ts`, `apps/cli/src/doctor.test.ts`.
...
- Web route server/component tests live beside routes with framework names stripped from test names: `apps/web/src/routes/boards/page.server.test.ts`, `apps/web/src/routes/boards/page.svelte.test.ts`.
- Web Playwright tests live under `apps/web/tests/e2e/` and `apps/web/tests/a11y/`.
- Vitest tests are selected by `apps/web/vitest.config.ts` include pattern `tests/vitest/**/*.test.ts`.
...
- Keep phase/parity names when tests encode milestone gates: `apps/server/src/api/__tests__/phase08-api-parity.test.ts`, `tests/parity/p13-three-surfaces.test.ts`.
...
**Patterns:**
- Use `describe()` around domain/route behavior and `test()` or `it()` for behavior names: `src/router/rules-engine-hot-reload.test.ts`, `apps/web/src/routes/boards/page.server.test.ts`.
- Use `beforeEach`/`afterEach` for isolated temp homes, PGlite stores, and environment mutation: `apps/web/src/routes/runs/page.server.test.ts`.
- Use `beforeAll`/`afterAll` for heavier shared setup such as ORM instances or dynamic Svelte SSR imports: `tests/db/migrator-service.test.ts`, `apps/web/src/routes/boards/page.svelte.test.ts`.
- Use cache-busted dynamic imports when testing SvelteKit server modules that read environment/module state: `apps/web/src/routes/boards/page.server.test.ts`.
- Use explicit data seeding helpers rather than hidden global fixtures for product DB tests: `seedTasks()` in `apps/web/src/routes/boards/page.server.test.ts`.
...
- Use Playwright fixtures for browser-level seeded product data: `apps/web/tests/e2e/_fixtures.spec.ts`.
...
- Keep route seed helpers in the route test file unless shared by multiple route suites: `seedTasks()` in `apps/web/src/routes/boards/page.server.test.ts`.
...
- Playwright browser fixtures live under `apps/web/tests/e2e/`, including fixture validation in `apps/web/tests/e2e/_fixtures.spec.ts`.

## Coverage

**Requirements:** Root Bun coverage threshold is 80% via `bunfig.toml`; web Vitest coverage threshold is 80% lines for configured include set in `apps/web/vitest.config.ts`.

**View Coverage:**
```bash
bun run scripts/test-root.ts --coverage
(cd apps/web && bun run web:test -- --coverage)
```
...
Use `await streamedData<T>(result)` for SvelteKit streamed load promises: `apps/web/src/routes/boards/page.server.test.ts`.
...
Use `rejects.toMatchObject()` for domain/framework errors where possible: `src/services/TaskService.test.ts`. Use explicit `try/catch` when framework helpers throw non-Error response objects: `apps/web/src/routes/runs/[id]/page.server.test.ts`.
