...
import {
  PHASE96_UAT_FIXTURE,
  assertPhase96FixtureIntegrity,
  phase96FixtureIds,
} from "./phase96-fixture.ts";

describe("Phase 09.6 canonical UAT fixture", () => {
  test("declares one shared Agent OS work cycle spine for every client", () => {
    expect(phase96FixtureIds()).toEqual({
  ...
  test("links human and agent journeys through the same trace records", () => {
    expect(PHASE96_UAT_FIXTURE.humanJourneys.map((journey) => journey.id)).toEqual([
      "H1",
      "H2",
      "H3",
      "H4",
    ...
    expect(PHASE96_UAT_FIXTURE.agentJourneys.map((journey) => journey.id)).toEqual([
      "A1",
      "A2",
      "A3",
      "A4",
    ...
    expect(PHASE96_UAT_FIXTURE.trace.sourceRefs).toEqual(
  ...
  test("fails closure when fixture has orphaned links or missing client coverage", () => {
    const result = assertPhase96FixtureIntegrity(PHASE96_UAT_FIXTURE);
