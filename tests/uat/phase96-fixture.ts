export type Phase96Client = "api" | "cli" | "tui" | "web";
...
type Phase96Journey = {
  id: string;
  title: string;
  requiredClients: Phase96Client[];
  refs: Phase96Ref[];
};

export type Phase96UatFixture = {
  ...
  trace: { causationId: string; sourceRefs: Phase96Ref[] };
  clientCoverage: Record<Phase96Client, string[]>;
  humanJourneys: Phase96Journey[];
  agentJourneys: Phase96Journey[];
};

export type Phase96FixtureIntegrity = {
  ok: boolean;
  missingClients: Phase96Client[];
  orphanReferences: Phase96Ref[];
  missingTraceKinds: Phase96RefKind[];
};
...
const workTaskRef: Phase96Ref = { kind: "work_item", id: ids.taskId };
const runRef: Phase96Ref = { kind: "run", id: ids.runId };
const artifactRef: Phase96Ref = { kind: "artifact", id: ids.artifactId };
const memoryRef: Phase96Ref = { kind: "memory", id: ids.memoryId };

export const PHASE96_UAT_FIXTURE: Phase96UatFixture = {
...
export function phase96FixtureIds(): typeof ids {
  return ids;
}

export function assertPhase96FixtureIntegrity(fixture: Phase96UatFixture): Phase96FixtureIntegrity {
  const knownRefs = new Set<string>();
  const add = (kind: Phase96RefKind, id: string) => knownRefs.add(`${kind}:${id}`);
  ...
  const missingClients = (Object.entries(fixture.clientCoverage) as Array<[Phase96Client, string[]]>)
    .filter(([, journeys]) => journeys.some((journey) => !coveredJourneys.has(journey)))
    .map(([client]) => client);
  const requiredTraceKinds: Phase96RefKind[] = [
    "workspace",
    "project",
    "repository",
    "work_item",
