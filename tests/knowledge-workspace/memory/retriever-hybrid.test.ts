...
import {
  ...
  normalizeBm25,
  hybridScore,
} from "@knowledge-workspace/application/memory/retrieval/hybrid-scoring.ts";
import {
  rankMemoryMatchesHybrid,
  type HybridMemoryRankInput,
} from "@knowledge-workspace/application/memory/retrieval/scoring.ts";
...
describe("rankMemoryMatchesHybrid", () => {
  test("hybrid re-ranks differently from FTS-only for ≥3/10 queries", () => {
    ...
    for (const query of queries) {
      ...
      const { rankMemoryMatches } = require("@knowledge-workspace/application/memory/retrieval/scoring.ts");
...
describe("sidecar fallback", () => {
  test("embedQuerySafe returns null on sidecar unavailable", async () => {
    const { embedQuerySafe } = await import("@knowledge-workspace/application/memory/retrieval/sidecar.ts");
