 ...
 *   8. actions.default() calls auth/sign-up/email + public accept-invite on unauthenticated path.
...
describe("invite accept +page.server actions.default() input validation", () => {
  ...
  it("returns fail(400) when accept-invite API call fails (unauthenticated path)", async () => {
    const mockFetch = async (url: string | URL | Request) => {
      const u = url.toString();
      if (u.includes("/api/auth/sign-up/email")) {
      ...
      if (u.includes("/api/v1/auth/accept-invite")) {
        return new Response(
  ...
  it("returns fail(400) when authenticated acceptInvite fails", async () => {
    const mockFetch = async (url: string | URL | Request) => {
      if (url.toString().includes("/api/v1/auth/accept-invite")) {
        return new Response(
          JSON.stringify({ message: "Invitation already used" }),
          { status: 400 },
        );
      }
      return new Response(null, { status: 500 });
