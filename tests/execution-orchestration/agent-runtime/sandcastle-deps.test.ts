...
describe("Sandcastle dependency policy", () => {
  ...
  test("keeps direct Effect imports inside orchestration modules", async () => {
    ...
    const violations = sourceFiles
      .filter((file) => !relative(root, file).startsWith("services/execution-orchestration/src/infrastructure/agent-runtime/"))
      .filter((file) => {
  ...
  test("exports the pinned Sandcastle API version from the sandbox runner stub", () => {
    const runnerPath = join(root, "services/execution-orchestration/src/infrastructure/agent-runtime/sandbox-runner.ts");
