...
describe("renderPrompt", () => {
  it("renders issue.title and nullable attempt in liquid strict mode", async () => {
    const { renderPrompt } = await import("@execution-orchestration/infrastructure/agent-runtime/symphony/prompt.ts");
  ...
  it("throws UnknownVariableError when template references unknown variables", async () => {
    const { UnknownVariableError, renderPrompt } = await import(
      "@execution-orchestration/infrastructure/agent-runtime/symphony/prompt.ts"
    );
...
describe("parseWorkflowConfig", () => {
  it("parses empty config as defaults", async () => {
    const { parseWorkflowConfig } = await import("@execution-orchestration/infrastructure/agent-runtime/symphony/prompt.ts");
