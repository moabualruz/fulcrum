...
import { MemoryRepository } from "../../src/db/repositories/memory/MemoryRepository.ts";
import { HeuristicExtractor } from "../../src/memory/extractor-heuristic.ts";
...
describe("HeuristicExtractor", () => {
  ...
  test("extracts at least one row of each kind from a mixed transcript", () => {
    ...
    const rows = extractor.extractMemories([
      "[read] src/memory/extractor-heuristic.ts",
      "decided: keep extraction side-effect free",
      "## Summary",
      "blocked by issue #12",
      "See [[Memory PRD]] and https://example.com/prd",
    ].join("\n"));
