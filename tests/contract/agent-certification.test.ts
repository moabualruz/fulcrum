...
describe("agent certification contract", () => {
  it("requires two real passing CLI agents and degrades unavailable agents with next actions", async () => {
    ...
    writeFileSync(
      path.join(root, "agent-one.json"),
      JSON.stringify({ schemaVersion: "1.0", result: { status: "passed" } })
    );
    writeFileSync(
      path.join(root, "agent-two.json"),
      JSON.stringify({ schemaVersion: "1.0", result: { status: "passed" } })
    );
    writeFileSync(
      path.join(root, "missing.json"),
      JSON.stringify({ schemaVersion: "1.0", result: { status: "guided" } })
    );
      ...
      acceptanceEvidence: [
        {
          agentId: "agent_one",
          status: "passed",
          evidenceRef: "agent-one.json",
          runId: "run_agent_one"
        },
        {
          agentId: "agent_two",
          status: "passed",
          evidenceRef: "agent-two.json",
          runId: "run_agent_two"
        },
    ...
    expect(report.certifications.find((item) => item.agentId === "agent_one")?.status).toBe(
      "certified"
    );
    expect(
      report.certifications.find((item) => item.agentId === "agent_one")?.acceptanceRunIds
    ).toEqual(["run_agent_one"]);
    expect(report.certifications.find((item) => item.agentId === "agent_missing")?.status).toBe(
      "degraded"
    );
