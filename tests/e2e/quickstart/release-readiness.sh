...
node -e "const fs=require('node:fs'); const p=JSON.parse(fs.readFileSync(process.argv[1], 'utf8')); if (p.schemaVersion !== '1.0') process.exit(1); if (!Array.isArray(p.failures)) process.exit(1);" "$EVIDENCE_DIR/release-evidence.json"
