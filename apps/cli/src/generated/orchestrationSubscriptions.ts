...
export function createOrchestrationSubscriptionsCommand(): Command {
  ...
  statusCommand.description("orchestrationSubscriptions status");
  statusCommand.option("--json", "Emit JSON output");
  statusCommand.action(async (options) => {
    try {
      throw new Error("Generated tRPC invocation for orchestrationSubscriptions.status requires an explicit surface adapter.");
    } catch (error) {
      if (options.json === true) {
        const message = error instanceof Error ? error.message : String(error);
        console.log(JSON.stringify({ error: { code: "INTERNAL_ERROR", message } }));
