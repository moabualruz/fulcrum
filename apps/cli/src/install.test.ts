...
describe("dry-run mode", () => {
  ...
  test("install --profile full keeps the old full bootstrap surface explicit", async () => {
    const proc = Bun.spawn(["bun", "apps/cli/src/main.ts", "install", "--dry-run", "--profile", "full"], {
      cwd: join(__dirname, "../.."),
      stdout: "pipe",
      stderr: "pipe",
      env: {
  ...
  test("install --profile rules-only only plans the global rules block", async () => {
    const proc = Bun.spawn(["bun", "apps/cli/src/main.ts", "install", "--dry-run", "--profile", "rules-only"], {
      cwd: join(__dirname, "../.."),
      stdout: "pipe",
      stderr: "pipe",
      env: {
