...
- Flat OD workbench routes are backwards-compatible bookmarks only. They redirect to canonical project-scoped stage URLs using the fallback scope `mkh/fulcrum`; production navigation and stage links stay under `/<ws>/projects/<projId>/<stage>[/<sub>]`.
