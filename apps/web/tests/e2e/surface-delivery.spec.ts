...
test.describe("Surface — Web surface delivery UAT", () => {
  ...
  test("task CRUD", async ({ page, fulcrumHome }) => {
    const project = await fulcrumHome.seedProject("surface-task-crud", "Surface Task CRUD");
    await fulcrumHome.seedTask({ projectId: project.id, title: "Surface Task", status: "pending" });
  ...
  test("kanban move", async ({ page, fulcrumHome }) => {
    const project = await fulcrumHome.seedProject("surface-kanban", "Surface Kanban");
  ...
  test("sprint management", async ({ page, fulcrumHome }) => {
    const project = await fulcrumHome.seedProject("surface-sprints", "Surface Sprints");
  ...
  test("doc editing", async ({ page, fulcrumHome }) => {
    const project = await fulcrumHome.seedProject("surface-doc-edit", "Surface Doc Edit");
    const doc = await fulcrumHome.seedDoc({ projectId: project.id, title: "Surface Doc", body: "Body" });
  ...
  test("search and facets", async ({ page }) => {
    await gotoAndExpectOk(page, "/search?q=surface");
  ...
  test("artifact download", async ({ page, fulcrumHome }) => {
    const project = await fulcrumHome.seedProject("surface-artifact", "Surface Artifact");
    const artifact = await fulcrumHome.seedArtifact({ projectId: project.id, title: "surface.txt", mime: "text/plain", size: 7 });
