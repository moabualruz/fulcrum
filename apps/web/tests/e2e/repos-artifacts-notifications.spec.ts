/**
 * E2E: Repository Artifact Notification - Repos, Artifacts, Notifications.
 *
 * Guarded so root `bun test` can load this file without starting Playwright.
 *
 * Run via:
 *   cd apps/web && bunx playwright test tests/e2e/repos-artifacts-notifications.spec.ts
 */
...
if (isPlaywrightCli) {
  ...
  test.describe("end-to-end artifact workflow cycle", () => {
    ...
    test("artifact detail exposes provenance and download behavior", async ({ page, fulcrumHome }) => {
      const project = await fulcrumHome.seedProject("repository-artifact-notification-artifacts", "Repository Artifact Notification Artifacts");
      const artifact = await fulcrumHome.seedArtifact({
        projectId: project.id,
        title: "repository-artifact-notification-report.txt",
        mime: "text/plain",
        size: 12,
      });

      const response = await page.goto(`/artifacts/${artifact.id}`);
      test.skip((response?.status() ?? 200) >= 500, "@test-support/product-workspace-fixtures.ts");
      ...
      await expect(title).toContainText("repository-artifact-notification-report.txt");
