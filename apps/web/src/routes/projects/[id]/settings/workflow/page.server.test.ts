...
function loadEvent(fetch: typeof globalThis.fetch, overrides: Partial<App.Locals> = {}) {
  return {
    ...
    fetch,
    request: new Request("http://localhost/projects/project-workflow/settings/workflow", {
      headers: { cookie: "sid=session-1" },
    }),
    url: new URL("http://localhost/projects/project-workflow/settings/workflow"),
  };
}

describe("project workflow settings route", () => {
  test("loads project context through the public project API", async () => {
    const calls: Array<{ url: string; init?: RequestInit }> = [];
    const fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      calls.push({ url: String(input), init });
      return new Response(JSON.stringify({ id: "project-workflow" }), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    }) as typeof globalThis.fetch;
    const mod = await import(`./+page.server.ts?project-workflow-load=${Date.now()}`);
    ...
    expect(calls[0]?.url).toBe("http://localhost/api/v1/projects/project-workflow?orgId=org-1");
  ...
  test("maps missing project API response to a route 404", async () => {
    ...
    const mod = await import(`./+page.server.ts?project-workflow-missing=${Date.now()}`);
