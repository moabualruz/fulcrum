...
- **Self-tests** via `bun run ci` — install → tsc → full Bun test suite → 5 platform builds → skills:lint → compress:check (hard gate, 0 pending). Latest package-surface gate on 2026-04-30: focused parity/status tests `50 pass`; live remove/install rebuilt Caveman, Repomix, Cloudflare, and Superpowers through `fulcrum component`; `component status package.* --json` reports every managed package/agent pair `ok:true`, zero missing surfaces, zero source-backup leaks, and one explicit unsupported Pi Repomix explorer-agent primitive. Targeted cleanup removed stale dropped-package caches/logs and old markerless Caveman fallback dirs; CLI roots contain no `.original.md`, `.backup.md`, or dropped-package paths. Disabled MCP smoke confirms context7/github disabled on all agents, with disabled config native for Codex/Gemini/OpenCode and `disabledConfigUnsupported` for Claude/Pi. Final full CI passed after docs updates (`bun run ci`: install/typecheck/test/build:all/skills:lint/compress:check all green).
