...
- **Manages MCPs** (`fulcrum mcp list/register/unregister/enable/disable [--all]`) via a TOML registry at `~/.fulcrum/state/global/mcp-registry.toml`. DeepWiki is now a normal registry builtin (not a special-case installer). Per-agent `applyToAgents` writes the canonical MCP shape with bearer auth wired:
  ...
  Minimal default state enables DeepWiki + context7 where no user state exists. `fulcrum install --no-default-mcps` registers all MCP definitions/config without changing enable state; `fulcrum install --enable-all-mcps` is a verification switch that explicitly flips every builtin on across every detected agent.

- **Validates the environment** (`fulcrum doctor [--json] [--probe]`) — agent detection, rules-spliced state, caveman per-agent install + `defaultMode` source, Pi MCP adapter check, component lifecycle counts, package parity reports, 47 BYO tools, tool-output policy presence, active skill metadata budget per agent, and project-local ignored worktree roots (`.claude/worktrees`). MCP registry section reports `auth_status` (env-var presence), `reachable` (HEAD probe / `which`), `drift` (`default_enabled=false` but a non-package-default server has some agent enabled), and `wiring` (per-agent native config inspection — confirms `bearer_token_env_var` / `headers.Authorization` is present for HTTP servers with declared auth). With `--probe`: spawns stdio MCPs / POSTs HTTP MCPs and asserts a valid JSON-RPC `initialize` reply within an 8s timeout. Catches wrong commands, stale URLs, and broken auth in one check.

- **Self-tests** via `bun run ci` — install → tsc → full Bun test suite → 5 platform builds → skills:lint → compress:check (hard gate, 0 pending). Latest cleanup/install gate on 2026-04-30: focused package/cleanup tests `48 pass`; real `fulcrum uninstall --purge --include-caveman` exposed cleanup gaps, then targeted code/tests closed dry-run mutation, state/cache purging, Claude marketplace cleanup, orphan Codex MCP blocks, package cache roots, Graphify pin drift, and Caveman source-manifest drift. Final real `fulcrum install --profile full` reinstalled the recommended setup. Per-package `component status package.<id> --json` reports Caveman, Repomix, Cloudflare, and Superpowers all `ok:true` across managed agents, zero missing surfaces, zero source-backup leaks, and one explicit unsupported Pi Repomix explorer-agent primitive. Disabled MCP smoke confirms context7/github disabled on all agents, with disabled config native for Codex/Gemini/OpenCode and `disabledConfigUnsupported` for Claude/Pi. Final full CI passed after cleanup fixes (`bun run ci`: install/typecheck/test/build:all/skills:lint/compress:check all green).
...
│                              builtins, minimal MCP defaults (deepwiki+context7).
...
│                              vendor packages, and 17 builtin MCPs.
...
│                            — Claude Code: plugin install (fulcrum@fulcrum
│                              from moabualruz/fulcrum marketplace); others:
│                              native mirrors under fulcrum/<name>/, with Codex
│                              global scope opt-in. Generated mirrors exclude
│                              .original.md/source-only files. Migrates legacy
...
│                            — inventory + eval coverage, or installed skill
│                              metadata budget by agent.
...
│   ├── package-surfaces.ts + .test.ts  # package surface discovery + hashes
│   ├── package-mirror.ts + .test.ts    # per-agent package target planner
│   ├── package-parity.ts + .test.ts    # source vs installed package audit
│   ├── mcp.ts                          # legacy DeepWiki/Pi compatibility helpers
│   ├── mcp-registry.ts + .test.ts      # TOML registry + applyToAgents
│   ├── mcp-builtins.ts                 # 17 builtin MCP specs (single source)
│   ├── mcp-cmd.ts + .test.ts           # fulcrum mcp CLI verbs
│   ├── doctor.ts + .test.ts            # health report w/ MCP + skill budget sections
│   └── …
└── hooks/
...
- `fulcrum install` — defaults to minimal profile (rules + policy + registry + DeepWiki/context7); `--profile full` keeps the old full sweep across 5 agents; doctor verdict ok on a fully-set-up machine.
- `fulcrum install --enable-all-mcps` — flips every builtin MCP on across every detected agent immediately after registration. End-to-end live verified 2026-04-29: 16/16 MCPs `handshake:ok` via `fulcrum doctor --probe`; current registry has 17 builtins after adding DeepWiki. Default install state enables only DeepWiki + context7 from the builtin registry where no user state exists; skip via `--no-default-mcps`.
...
- `fulcrum doctor` — verdict ok on this machine; per-agent state, caveman across 5 agents, Pi MCP adapter, skill metadata budget, project-local worktree warnings, drift detection, auth-wiring inspection.
...
- `bash scripts/install.sh [--dry-run] [--profile minimal|rules-only|full] [--with-project DIR] [--no-skills] [--no-upstream-skills] [--no-default-mcps] [--enable-all-mcps]` — flag pass-through verified bash 3.2-safe.
...
- [x] `subagent-orchestration` skill added, split into reference files, mirrored into the Fulcrum plugin, compressed, linted, synced, then hardened for max-useful parallelism, external worktree write lanes, runtime dependency reassessment, and lane-specific model/effort selection.
- [x] Subagent orchestration evals accepted for now across target agents. Codex/OpenCode passed full statistical evals; Claude/Pi/Gemini findings are accepted as non-blocking harness/runtime quirks for this branch. Latest Codex spot eval was stopped after user acceptance at 10/12 positive triggers + first negative clean.
...
- [x] Plugin/extension/package surface parity implemented: shared manifest/mirror/parity modules, full package mirrors for Caveman/Repomix/Cloudflare/Superpowers, disabled MCP setup semantics, project integration component visibility, live cleanup/reinstall verification, and stale dropped-package artifact removal.
...
| Pi DeepWiki via Fulcrum-managed `pi-mcp-adapter` | Pi has no built-in MCP manager; adapter bridges registry-owned DeepWiki and other MCPs into Pi. | `src/cli/mcp-builtins.ts`, `src/cli/mcp-registry.ts`, `docs/mcp.md` §3.3 |
...
### Build order + mainline plan

| # | Execution | Layer | Depends |
|---|---|---|---|
| 0 | `main` + subagent orchestration | Component lifecycle management (`fulcrum component …`) | foundation |
| 1 | `main` after step 0 | §6.1 | foundation |
| 2 | `main` after step 0 | §6.2 | foundation (parallel-safe with 1) |
| 3 | `main` after 1, 2 | §6.3 | 1, 2 |
| 4 | `main` after 1, 2, 3 | §6.4 | 1, 2, 3 |
| 5 | `main` after 1–4 | §6.5 | 1–4 |
| 6 | `main` after 1, 3, 4 | §6.6 | 1, 3, 4 (parallel-safe with 5) |
| 7 | `main` after 1–6 | §6.7 | 1–6 |
...
Default install is minimal: splices rules, seeds policy, registers 17 builtin MCPs, and enables DeepWiki + context7 where no user state exists. `--profile full` adds hook snippets, caveman per agent (Claude/Gemini vendor commands; Codex/OpenCode/Pi direct official repo mirrors), 29 authored + 19 upstream skills with subpath SHA-256, and vendor packages with full package-surface mirrors where native installers are unavailable. Codex authored skills are not globally synced unless explicitly requested; generated CLI agent folders exclude `.original.md` / `.backup.md` while project source keeps those backups.
...
Default install state: every builtin MCP is registered; DeepWiki + context7 are enabled as minimal defaults only where no user state exists; other builtin MCPs stay disabled (many active MCPs = ~150–300k tokens at session start). Use `--no-default-mcps` for registry-only/no-state-change setup, opt in selectively, or use `--enable-all-mcps` for verification.
...
2. **Optionally revisit eval precision after the next implementation pass.**
...
fulcrum mcp list --json                 # 17 builtin servers
