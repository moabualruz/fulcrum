...
- **Validates the environment** (`fulcrum doctor [--json] [--probe]`) — agent detection, rules-spliced state, caveman per-agent install + `defaultMode` source, Pi MCP adapter check, 47 BYO tools, tool-output policy presence, active skill metadata budget per agent, and project-local ignored worktree roots (`.claude/worktrees`). MCP registry section reports `auth_status` (env-var presence), `reachable` (HEAD probe / `which`), `drift` (`default_enabled=false` but a non-package-default server has some agent enabled), and `wiring` (per-agent native config inspection — confirms `bearer_token_env_var` / `headers.Authorization` is present for HTTP servers with declared auth). With `--probe`: spawns stdio MCPs / POSTs HTTP MCPs and asserts a valid JSON-RPC `initialize` reply within an 8s timeout. Catches wrong commands, stale URLs, and broken auth in one check.

- **Self-tests** via `bun run ci` — install → tsc → full Bun test suite → 5 platform builds → skills:lint → compress:check (hard gate, 0 pending). Latest component lifecycle gate green on 2026-04-30: component-focused suite `106 pass`, affected legacy suite `197 pass`, full `bun run ci` six-stage gate, and dry-run smokes for component list/plan, hook install, install wrapper, and uninstall wrapper. Latest cleanup verification on 2026-04-30: targeted shell inspection found old markerless Claude plugin caches; `fulcrum uninstall --purge --include-caveman` now removes those leftovers through project code, `fulcrum install --profile full` reinstalls all five agent surfaces, and checked CLI roots contain no `.original.md`. Latest Repomix package audit on 2026-04-30: `bun run src/index.ts component remove package.repomix` then `bun run src/index.ts component install package.repomix` rebuilt all five Repomix agent surfaces; direct checks confirmed Codex plugin cache + config + MCP, Gemini extension MCP/commands/rules, OpenCode/Pi enabled MCP, five mirrored skills per non-Claude skill root, and no `.original.md` under generated CLI agent folders; final `bun run ci` passed all six stages (`450 pass` tests).
...
Default install is minimal: splices rules, seeds policy, registers 17 builtin MCPs, and enables DeepWiki + context7 where no user state exists. `--profile full` adds hook snippets, caveman per agent (Claude/Gemini vendor commands; Codex/OpenCode/Pi direct official repo mirrors), 29 authored + 27 upstream skills with subpath SHA-256, and vendor packages. Codex authored skills are not globally synced unless explicitly requested; generated CLI agent folders exclude `.original.md` while project source keeps those backups.
