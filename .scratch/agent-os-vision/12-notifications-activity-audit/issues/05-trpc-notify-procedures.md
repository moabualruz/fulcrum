...
## Acceptance criteria
- [ ] Schema migration: reads/writes all notification tables.
...
- `01-schema-migration.md` — all tables.
...
- `notify.unreadCount`: `SELECT COUNT(*) FROM user_notifications WHERE user_id=$1 AND read_at IS NULL`; cache-busted on `markRead`/`markAllRead`.
