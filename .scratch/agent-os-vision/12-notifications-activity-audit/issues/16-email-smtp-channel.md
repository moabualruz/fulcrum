...
## What to build
When `FULCRUM_FEATURES=notify-email` ON: register `notify-deliver-email` graphile-worker task; sends via `nodemailer` transport with Eta v3 template; writes `notification_deliveries` row with `status='sent'|'failed'`; rate limiter (>N/hr → `status='suppressed'`); email verify flow: token generated on settings save, confirm link sets `email_verified=true` on user, unverified → delivery suppressed. Flag OFF → no SMTP calls.

## Acceptance criteria
- [ ] Schema migration: reads `notification_deliveries`; reads/writes `users.email_verified`.
- [ ] tRPC procedure / module: `notify-deliver-email` graphile-worker task + transport factory in `src/notifications/channels/email.ts`; `notify.channels.config` for SMTP settings.
...
- Rate limiter: `SELECT COUNT(*) FROM notification_deliveries WHERE user_id=$1 AND channel='email' AND created_at > now() - interval '1 hour'`; if >N → `status='suppressed'`.
