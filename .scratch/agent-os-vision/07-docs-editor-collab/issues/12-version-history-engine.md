...
## Acceptance criteria
- [x] `version-writer.ts`: every `docs.update` writes a `doc_versions` row; `version_num` auto-increments per doc
- [x] `version-writer.ts`: rows 1 and every Nth (default 10) + first save of each calendar day → `snapshot` populated; all other rows → `delta` populated; `body_md_snapshot` always populated
- [x] `version-reconstructor.ts`: reconstruction of any version = nearest prior snapshot + sequential delta application; result matches original `content_json` byte-stably
- [x] `docs.versions.list`: returns rows DESC with `{id, version_num, is_snapshot, author_id, created_at}`; org-scoped
- [x] `docs.versions.diff`: accepts `from_version_num` + `to_version_num`; returns jsondiffpatch HTML visual diff string
- [x] `docs.versions.restore`: creates new `doc_versions` row with reconstructed content; sets `restore_of` FK; does NOT overwrite existing rows; `docs.update` called with restored `content_json`+`body_md`
- [x] Performance: version restore (50 versions, 5 snapshots) < 150 ms on PGlite
- [x] Tests: 12 saves → 12 rows; rows 1 and 10 have snapshot; rows 2-9, 11-12 have delta only
- [x] Tests: restore v5 → new row with `restore_of=v5`; reconstructed `content_json` + `body_md` byte-equal to original v5
- [x] Tests: jsondiffpatch failure gate — docs > 500 kB: if delta > 200 ms, writer falls back to full snapshot mode (no delta computed)
