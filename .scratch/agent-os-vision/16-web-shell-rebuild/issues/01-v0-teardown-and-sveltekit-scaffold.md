...
Delete `src/web/src/routes/` v0 admin routes entirely. Scaffold the new SvelteKit 2 (Svelte 5 runes) app structure under `src/web/src/`. Establishes the `+layout.svelte` app shell with sidebar nav, topbar, theme-var injection, toast provider, and cmd+K portal mount. Wires `+layout.server.ts` for session hydration and feature-flag hydration. Integrates Better-Auth `hooks.server.ts` with auto-redirect unauthenticated requests to `/auth/login`. Mounts the tRPC client at `src/web/src/lib/trpc.ts`. Updates `bun run ci` to point at the new build path. Adds a `docs/web-v0-migration.md` mapping old routes to new.

Cuts through: file system (delete v0) → SvelteKit layout hierarchy → `hooks.server.ts` session context → tRPC client wiring → `bun run ci` gate → Playwright smoke.
...
- [ ] `+layout.server.ts`: loads `event.locals.session` + `event.locals.featureFlags`; unauthenticated → redirect `/auth/login` (except `/auth/*` and `/api/*`).
- [ ] `hooks.server.ts`: Better-Auth session validated per request; `event.locals.user` populated on valid session.
