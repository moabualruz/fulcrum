...
- [x] tRPC procedure / module: `src/artifacts/worker.ts` exports `registerArtifactWorkerTasks(worker)` called from main worker bootstrap.
...
- [x] Tests: unit — worker task calls `harvestArtifacts()` with correct args; error thrown → job retried (graphile-worker retry contract); enqueue shim returns without awaiting harvest; doctor reports 0 pending on clean run; RED→GREEN.
