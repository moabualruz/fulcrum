...
## Acceptance criteria
- [x] Schema migration: no new columns; `artifacts.path` stores S3 key (same relative path convention).
- [x] tRPC procedure / module: `createStorageBackend()` factory returns correct backend per enabled flag; `artifacts.upload` / harvest use factory.
- [x] Web surface: `/artifacts/<id>/download` redirects to presigned URL when S3 flag ON; local-fs download otherwise. Flag off → local-fs only (no regression).
- [x] CLI command: `fulcrum artifacts download <id> --out /tmp/x` works with S3 backend (streams presigned URL); `--json` includes `storageBackend: 's3'` in metadata.
- [x] TUI screen: `d` download uses presigned URL stream when S3 flag ON.
- [x] Tests: S3 backend tested against MinIO (local Docker or `@aws-sdk` mock); flag OFF → local-fs only, no S3 calls; ON → file in MinIO bucket, not on local disk; download → presigned redirect; Azure + GCS backend unit tests with SDK mocks; retry 3× on network error; RED→GREEN.
