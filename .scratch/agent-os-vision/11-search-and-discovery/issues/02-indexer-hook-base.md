...
## What to build
`src/search/indexers/base.ts` — abstract `SearchIndexHook` with `upsert(entityId, orgId): Promise<void>` and `remove(entityId, orgId): Promise<void>`. `upsert` writes/overwrites `search_documents` row via `ON CONFLICT (org_id, kind, entity_id) DO UPDATE`; `remove` deletes row. Also implements `bulkReindex(orgId, kind)` graphile-worker task for re-indexing all entities of a kind (used on flag enable or migration). Validates `ts_vector` is populated post-upsert via DB query assertion in tests.
...
## Blocked by
- `01-schema-migration.md` — `search_documents` DDL.
...
- Failure gate: if `tsvector GENERATED ALWAYS AS STORED` not supported by PGlite version → implement trigger-based maintenance.
