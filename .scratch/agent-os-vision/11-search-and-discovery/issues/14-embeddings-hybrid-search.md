...
## Acceptance criteria
- [ ] Schema migration: `embedding vector(384)` column already in `0011_search` (NULL when flag OFF); IVFFlat index created on flag enable (idempotent `CREATE INDEX IF NOT EXISTS`).
