...
**i18n** (`FULCRUM_FEATURES=i18n`): Settings → i18n screen (available locale list; `Enter` selects → calls `TenantSettingsRepository.set(locale)` through an injectable settings service; TUI re-renders all labels from paraglide-js message catalog). When flag OFF, screen route hidden; navigation attempt → "Feature disabled" banner.
...
Both features: tested OFF (screen/toggle hidden) and ON (feature active). State persisted through `TenantSettingsRepository`.
...
- [ ] i18n ON: locale list renders; selecting `fr` → `TenantSettingsRepository` stores locale `fr`; TUI labels switch to French (test with at least 3 translated strings).
