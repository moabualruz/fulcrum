...
## Acceptance criteria
- [ ] Schema / state machine: `nativeUpdate` with `orchestrationState:'unclaimed'` filter references `agent_runs_claimed_unique` partial index; second concurrent claim on same task returns `ClaimConflictError`
- [ ] Tracker adapter: N/A
