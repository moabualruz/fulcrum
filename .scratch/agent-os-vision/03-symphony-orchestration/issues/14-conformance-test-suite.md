...
## Acceptance criteria
- [x] Schema / state machine: claim-lock test, state-enum test, partial-index test all present
- [x] Tracker adapter: `fetchCandidateIssues` ordering test, `fetchIssuesByStates` batch test, `fetchIssueStatesByIds` slim-shape test
- [x] Dispatch loop / hooks: dispatch happy-path test, retry formula parameterized table test, stall detection mocked-clock test, hook-timeout test
- [x] Surfaces (web/cli/tui parity): N/A (test suite is standalone)
- [x] Tests: `bun test src/orchestration/__tests__/symphony-conformance.test.ts` exits 0 with zero `.todo`/`.skip` items; CI step asserts exit code 0; total test count ≥ number of REQUIRED SPEC.md sections
- [x] SPEC conformance traced in `docs/symphony-conformance.md`: every REQUIRED section has at least one test ID mapped in conformance doc
