...
ORM/DB: MikroORM v7 (`@mikro-orm/core` + `mikro-orm-pglite` driver in local mode, `@mikro-orm/postgresql` in SaaS mode). Entities live at `src/db/entities/orchestration/`. Repositories extend `EntityRepository<T>` at `src/db/repositories/orchestration/`. Migrations auto-generated by `mikro-orm migration:create` at `src/db/migrations/Migration<timestamp>.ts` — never hand-written. DI via `needle-di` Stage-3 decorators; services annotated `@Injectable()`. See C7, C8, C9 in DECISIONS.md.
...
All schema artifacts are MikroORM v7 `@Entity` classes (C6, C7, C9). No `.sql` files; no hand-written DDL. Migration classes are auto-generated by `mikro-orm migration:create` and live at `src/db/migrations/Migration<timestamp>.ts`.

### `AgentRun` entity — Symphony state columns (additive; full entity in Pillar 4)

```typescript
// src/db/entities/orchestration/AgentRun.ts  (additive properties added by P3 migration)
@Property({ type: 'string', nullable: true,
  check: `orchestration_state IN ('unclaimed','claimed','running','retry_queued','released','succeeded','failed','timed_out','stalled','cancelled')` })
orchestrationState?: string;  // D1: generic name, not symphony_state
...
Partial indexes (emitted inside the auto-generated migration class body via `addSql()`):
- `agent_runs_claimed_unique` — unique on `(task_id) WHERE orchestration_state = 'claimed'`
- `agent_runs_dispatch_poll` — `(org_id, orchestration_state, next_retry_at) WHERE orchestration_state IN ('unclaimed','retry_queued')`
- `agent_runs_stall_scan` — `(org_id, orchestration_state, started_at) WHERE orchestration_state = 'running'`

### `Task` entity — eligibility columns (additive)

```typescript
// src/db/entities/orchestration/Task.ts  (additive properties added by P3 migration)
...
Partial index: `tasks_dispatch_eligible` — `(org_id, status, priority, created_at) WHERE status = 'ready'`.

### `WorkflowDefinition` entity (new)
...
export class WorkflowDefinition {
  ...
  project?: Project;  // NULL = org-wide default

  @Property({ type: 'string' })
  name!: string;

  @Property({ type: 'text' })
  configYaml!: string;  // WORKFLOW.md YAML front-matter block

  @Property({ type: 'text' })
  promptMd!: string;    // WORKFLOW.md Markdown body
...
Repository: `src/db/repositories/orchestration/WorkflowDefinitionRepository.ts`
...
sequenceDiagram
    ...
    Tracker->>DB: taskRepo.find({ status:'ready', blockedByIds:[] }, { orderBy: {priority:'asc',createdAt:'asc',id:'asc'} })
    DB-->>Tracker: candidate rows
    Tracker-->>Orch: Task[]

    loop per task
        Orch->>DB: agentRunRepo.nativeUpdate({ taskId, orchestrationState:'unclaimed' }, { orchestrationState:'claimed' })
        alt claim conflict
            ...
            Orch->>DB: agentRunRepo.assign(run, { orchestrationState:'succeeded' }); em.flush()
            Orch->>DB: eventsRepo.create({ verb:'state_changed', ... }); em.flush()
        end
    end
```
...
6. **`orchestration.active-runs.stalled`** — `agentRunRepo.count({ orchestrationState: 'stalled' })`; warn if > 0, fail if > 10.
...
- `orchestration.active-runs.stalled fail` → `fulcrum symphony runs list --state stalled --json`; `fulcrum symphony runs retry <id>`. (Count via `agentRunRepo.count({ orchestrationState:'stalled' })`)
...
| P3.2 | `workflow_definitions` migration | `WorkflowDefinition` entity absent → migration class → table + composite indexes present | `mikro-orm migration:create` generates `Migration<timestamp>`; verify `(org_id, project_id, name)` + `(org_id, project_id)` indexes |
| P3.3 | `agent_runs` Symphony columns | five new properties absent → migration class → present with CHECK constraints | additive migration class via `mikro-orm migration:create`; partial indexes for dispatch poll + stall scan + claimed unique |
