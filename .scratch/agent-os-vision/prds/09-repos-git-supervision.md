...
### Schema layer
Migration class covering `0009_repos_git` scope. Full entity definitions in §Schema changes below. Entities: `Repo` (extended), `RepoBranch`, `RepoCommit`, `RepoFilesIndex`. Amendment: `Task.repo` FK. Composite `(org_id, …)` indexes on all entities per Q22.
...
ORM/DB: MikroORM v7 (`@mikro-orm/core` + `mikro-orm-pglite` local, `@mikro-orm/postgresql` SaaS). Entities: `src/db/entities/repos/`. Repositories: `src/db/repositories/repos/`. Migrations auto-generated by `mikro-orm migration:create` at `src/db/migrations/Migration<timestamp>.ts`. chokidar watcher + `simple-git` wrapper are `@Injectable()` TS services — no ORM involvement. DI via `needle-di`. See C7, C8, C9 in DECISIONS.md.
...
All schema artifacts are MikroORM v7 `@Entity` classes (C6, C7, C9). Migration class covering `migration covering 0009_repos_git` auto-generated by `mikro-orm migration:create`. No `.sql` files.

### `Repo` entity — additive properties (`src/db/entities/repos/Repo.ts`)

```typescript
@Property({ type: 'string', default: '' }) name: string = '';
@Property({ type: 'string', default: '' }) slug: string = '';
@Property({ type: 'string', default: 'local',
  check: `kind IN ('local','remote')` })
kind: string = 'local';
@Property({ type: 'string', nullable: true }) localPath?: string;
@Property({ type: 'string', nullable: true }) remoteUrl?: string;
@Property({ type: 'string', default: 'main' }) defaultBranch: string = 'main';
@Property({ type: 'string', default: 'main' }) currentBranch: string = 'main';
@Property({ type: 'date', nullable: true }) lastSyncAt?: Date;
@Property({ type: 'string', default: 'idle',
  check: `sync_status IN ('idle','syncing','error')` })
syncStatus: string = 'idle';
@Property({ type: 'date', defaultRaw: 'now()' }) lastTouchedAt: Date = new Date();
@Property({ type: 'boolean', default: false }) archived: boolean = false;
```

Indexes: unique `repos_org_slug (org_id, slug)`; `repos_org_touched (org_id, last_touched_at DESC)`; `repos_kind_status (kind, sync_status)`.

### `RepoBranch` entity (new — `src/db/entities/repos/RepoBranch.ts`)

```typescript
@Entity()
export class RepoBranch {
  @PrimaryKey({ type: 'string' }) id!: string;
  @ManyToOne(() => Repo, { onDelete: 'cascade' }) repo!: Repo;
  @ManyToOne(() => Org) org!: Org;
  @Property({ type: 'string' }) name!: string;
  @Property({ type: 'string', nullable: true }) headSha?: string;
  @Property({ type: 'boolean', default: false }) isDefault: boolean = false;
  @Property({ type: 'boolean', default: false }) isCurrent: boolean = false;
  @Property({ type: 'date', defaultRaw: 'now()' }) lastSeenAt: Date = new Date();
}
```

Unique: `(repo_id, name)`. Index: `repo_branches_org_repo (org_id, repo_id)`.

### `RepoCommit` entity (new — `src/db/entities/repos/RepoCommit.ts`)
...
export class RepoCommit {
  ...
  @ManyToOne(() => Repo, { onDelete: 'cascade' }) repo!: Repo;
  @ManyToOne(() => Org) org!: Org;
  @Property({ type: 'string' }) sha!: string;
  @Property({ type: 'string', nullable: true }) authorName?: string;
  @Property({ type: 'string', nullable: true }) authorEmail?: string;
  @Property({ type: 'date' }) committedAt!: Date;
  @Property({ type: 'string', nullable: true }) subject?: string;
  @Property({ type: 'text', nullable: true }) body?: string;
  @Property({ type: 'array', nullable: true }) parents?: string[];
}
```

Unique: `(repo_id, sha)`. Indexes: `repo_commits_repo_date (repo_id, committed_at DESC)`; `repo_commits_org_repo (org_id, repo_id)`.

### `RepoFilesIndex` entity (new — `src/db/entities/repos/RepoFilesIndex.ts`)
...
export class RepoFilesIndex {
  ...
  @Property({ type: 'string', default: 'file',
    check: `kind IN ('file','dir')` })
  kind: string = 'file';
  @Property({ type: 'bigint', nullable: true }) sizeBytes?: number;
  @Property({ type: 'date', nullable: true }) lastModified?: Date;
  @Property({ type: 'date', defaultRaw: 'now()' }) lastIndexedAt: Date = new Date();
}
```

Unique: `(repo_id, path)`. Index: `repo_files_org_repo (org_id, repo_id, kind)`.

### `Task` entity — `repo` FK amendment (additive)

```typescript
// src/db/entities/tasks/Task.ts  (additive property)
@ManyToOne(() => Repo, { nullable: true })
repo?: Repo;
```

Index: `tasks_org_repo (org_id, repo_id)`.

Repositories: `src/db/repositories/repos/{Repo,RepoBranch,RepoCommit,RepoFilesIndex}Repository.ts`.
...
sequenceDiagram
    ...
    GW->>DB: repoRepo.assign(repo, { syncStatus:'syncing' }); em.flush()
    GW->>SG: git.status()
    SG-->>GW: {branch, dirty, ahead, behind}
    GW->>DB: repoRepo.assign(repo, { currentBranch }); em.flush()
    GW->>SG: git.branch(['-a'])
    SG-->>GW: branch list
    GW->>DB: repoBranchRepo.upsertMany([...branches]); em.flush()
    GW->>SG: git.log(['--max-count=200'])
    SG-->>GW: commit log
    GW->>DB: repoCommitRepo.upsertMany([...commits]); em.flush()
    GW->>SG: git.raw(['ls-files','--stage'])
    SG-->>GW: file list with paths
    GW->>DB: repoFilesIndexRepo.upsertMany([...files]); em.flush()
    GW->>DB: searchDocRepo.upsertMany([...], { sourceKind:'repo_file' }); em.flush()
    GW->>DB: repoRepo.assign(repo, { lastSyncAt: new Date(), syncStatus:'idle' }); em.flush()
    GW->>DB: eventsRepo.create({ verb:'repo.sync.completed' }); em.flush()
```
...
|---|---|---|
| `repos.schema.migration` | `RepoBranch`, `RepoCommit`, `RepoFilesIndex` entity tables present | Run `mikro-orm migration:up` for P9 migration class |
...
1. Migration class covering `0009_repos_git` scope applies clean on PGlite + PostgreSQL; composite indexes present; `Task.repo` FK added without data loss.
