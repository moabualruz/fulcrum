...
All migrations idempotent. Composite `(org_id,…)` indexes on every table (Q22).

```sql
ALTER TABLE docs
  ADD COLUMN IF NOT EXISTS parent_id uuid REFERENCES docs(id) ON DELETE SET NULL,
  ...
  ADD COLUMN IF NOT EXISTS sort_position float8 NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS archived boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS external_id text;
CREATE INDEX docs_org_project_scope ON docs(org_id, project_id, scope);
CREATE INDEX docs_org_doc_type ON docs(org_id, doc_type);
CREATE INDEX docs_org_parent ON docs(org_id, parent_id);
CREATE UNIQUE INDEX docs_org_external_id ON docs(org_id, external_id)
...
CREATE TABLE doc_links (
  ...
  to_doc_id uuid REFERENCES docs(id) ON DELETE SET NULL,
...
CREATE TABLE doc_templates (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (org_id, project_id, doc_type, name)
);
...
sequenceDiagram
    ...
    TR->>DB: UPDATE docs SET content_json, body_md
    TR->>VW: write version row
    VW->>DB: SELECT max(version_num) WHERE doc_id
    alt version_num % SNAPSHOT_EVERY == 0
        VW->>DB: INSERT doc_versions(snapshot=content_json, body_md_snapshot)
    else
        VW->>DB: INSERT doc_versions(delta=jsondiffpatch(prev, curr))
    end
    TR->>WL: extractWikilinks(content_json)
    WL->>DB: DELETE doc_links WHERE from_doc_id
    WL->>DB: INSERT doc_links (bulk upsert)
    TR->>SI: upsert(docId)
    SI->>DB: UPSERT search_documents
...
|---|---|---|
| `docs.schema.doc_links` | `doc_links` table exists | Run migration P7-01/P7-02 |
| `docs.schema.doc_versions` | `doc_versions` table exists | Run migration P7-02 |
| `docs.schema.doc_comments` | `doc_comments` table exists | Run migration P7-02 |
| `docs.schema.doc_templates` | `doc_templates` table exists; defaults seeded | Run migration P7-04 |
...
**Schema + foundation**
- `P7-01` Migration: `docs` ALTER (parent_id, scope, doc_type, frontmatter, body_md, content_json, sort_position, archived, external_id) + all indexes.
- `P7-02` Migration: `doc_links`, `doc_versions`, `doc_comments`, `doc_templates` + indexes.
