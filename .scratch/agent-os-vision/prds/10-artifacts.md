...
### Schema layer
Migration `0010_artifacts` extends and tightens the existing `artifacts` table stub, adds retention columns, and creates the prune-jobs schedule.
...
### Harvest pipeline
`src/artifacts/harvest.ts` — `harvestArtifacts(runId, extractedDir)`:
...
4. Copy file to artifact store path; write `artifacts` row; write two `edges` rows.
5. Upsert `search_documents` row: `source_kind='artifact'`, `source_id=artifact.id`, `title=filename`, `body=<first 2000 chars if text-mime else empty>`.
6. Emit `events` row `verb='artifact.harvested'`.
...
### Retention + pruner
- `artifact.prune` graphile-worker cron (daily at 02:00): selects `artifacts WHERE retention_until < now() AND archived = false`; deletes files from disk; sets `artifacts.archived = true` (soft-delete first); after 7 days second pass hard-deletes rows.
...
```sql
-- migration 0010_artifacts

-- Extend artifacts (replaces stub)
ALTER TABLE artifacts
  ADD COLUMN IF NOT EXISTS mime              text        NOT NULL DEFAULT 'application/octet-stream',
...
-- Drop old stub columns if they exist with wrong names
-- (handled by checking information_schema in migration script)
...
sequenceDiagram
    ...
    loop each file in extractedDir
        ...
        HV->>DB: SELECT projects.artifact_retention_days
        HV->>HV: compute retention_until
        HV->>FS: put(orgSlug/projectSlug/runId/filename, fileBuffer)
        FS-->>HV: stored path (relative)
        HV->>DB: INSERT artifacts(checksum, mime, size, path, retention_until)
        HV->>DB: INSERT edges(artifact->generated_by->run)
        HV->>DB: INSERT edges(run->produced->artifact)
        HV->>DB: UPSERT search_documents(source_kind=artifact)
    end
    HV->>DB: INSERT events(verb=artifact.harvested)
...
|---|---|---|
| `artifacts.schema.migration` | `artifacts` table with extended columns present | Run migration 0010 |
...
| # | Title | Layer |
|---|---|---|
| 10-01 | Migration `0010_artifacts`: extend artifacts table, add indexes, projects.artifact_retention_days | DB |
...
1. Migration `0010_artifacts` applies clean on PGlite + PostgreSQL; all composite indexes present; `projects.artifact_retention_days` added.
