...
ORM/DB: MikroORM v7 (`@mikro-orm/core` + `mikro-orm-pglite` driver local, `@mikro-orm/postgresql` SaaS). Entities: `src/db/entities/sandbox/`. Repositories: `src/db/repositories/sandbox/`. Migrations auto-generated by `mikro-orm migration:create` at `src/db/migrations/Migration<timestamp>.ts`. DI via `needle-di` `@Injectable()` services. See C7, C8, C9 in DECISIONS.md.

Provider drivers (`noSandbox`, `docker`, `podman`, `vercel`, `daytona`, `modal`, `e2b`) are `@Injectable()` services — no raw SQL, no driver registry SQL.
...
All schema artifacts are MikroORM v7 `@Entity` classes (C6, C7, C9). No `.sql` files. Migration classes auto-generated by `mikro-orm migration:create`.

### `AgentRun` entity — Sandcastle columns (additive)

```typescript
// src/db/entities/sandbox/AgentRun.ts  (additive properties)
...
Index: `agent_runs_agent_org` — `(org_id, agent_name, status, created_at)`.
...
Indexes: `(org_id, run_id)`, `(org_id, task_id)`.
...
Indexes: unique `(org_id, from_kind, from_id, to_kind, to_id, kind)`; lookup `(org_id, to_kind, to_id, kind)`.

### `AgentProfile` entity (new)

```typescript
// src/db/entities/sandbox/AgentProfile.ts
@Entity()
export class AgentProfile {
  ...
  @Property({ type: 'integer', default: 600000 })
...
Repositories: `src/db/repositories/sandbox/{Artifact,Edge,AgentProfile}Repository.ts`.
...
sequenceDiagram
    ...
    SR->>DB: agentRunRepo.assign(run, { status:'running', sandboxMode:'host' }); em.flush()
    ...
    GIT-->>SR: diff patch
    SR->>DB: agentRunRepo.assign(run, { iterationCount, transcriptPath, workspaceDiffPath, exitCode }); em.flush()
...
| P4.15 | `AgentRun` Sandcastle properties migration | seven new properties absent post-migration | MikroORM migration class; `sandbox_mode` CHECK; composite index `(org_id, agent_name, status, created_at)` |
| P4.16 | `Artifact` + `Edge` entity migrations | tables absent | MikroORM migration classes; `(org_id, run_id)` index; unique `(org_id, from_kind, from_id, to_kind, to_id, kind)` |
| P4.17 | `AgentProfile` entity migration + test persistence | `last_tested_at` not written after `agents test` | MikroORM migration class; `agents.testProfile` mutation writes test result via `agentProfileRepo.assign(...)` + `em.flush()` |
