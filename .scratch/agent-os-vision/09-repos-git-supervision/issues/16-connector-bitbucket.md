...
- [ ] Migration class covering `0009d_bitbucket` scope: `BitbucketPr` entity + `BitbucketIssue` entity — standard property shape; `bbPrId` UUID primary key from Bitbucket API. `@Entity` classes at `src/db/entities/repos/`; migration class auto-generated by `mikro-orm migration:create`.
