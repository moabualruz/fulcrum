...
Implement all 11 `platform.*` doctor checks in the doctor subsystem (Pillar 1 extension point). Each check: returns `{name, status: 'pass'|'warn'|'fail', message, recovery, checked_at}` JSON shape per PRD Zod schema. Checks: `platform.theme` (tenant_settings readable, accent parseable HEX); `platform.keyring` (OS keyring reachable OR fallback file exists mode 0600); `platform.keyring_mode` (warn if fallback in use); `platform.credentials` (table exists + encryption round-trip succeeds); `platform.crashlog_dir` (`~/.fulcrum/state/errors/` exists + writable); `platform.backup_last_run` (last backup <7d or no policy — info only); `platform.telemetry` (`opted_in` has value); `platform.flags_registry` (feature-flag registry loads, count reported); `platform.experiment_table` (`experiment_assignment` table exists); `platform.i18n` (when `i18n` flag ON: all locale JSON present, zero missing keys); `platform.remote_backup` (when `scheduled-backups` ON: remote DSN reachable + test PUT succeeds).
