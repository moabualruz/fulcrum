...
- [ ] Secrets tab: add → name+value prompted; list shows masked values; `Enter` unmasks for 3s; `D` deletes `Credential`.
- [ ] Errors tab: scroll, expand, delete work; `C` clears all with confirmation.
- [ ] Backup tab: `B` → progress indicator shows KB written → completion message; restore flow shows preflight entity counts.
- [ ] Telemetry tab: toggle persists; `P` shows count before and after (should be 0).
- [ ] Flags tab: `Space` toggles `enabled`; `E` prompts rollout %; change persists through `FeatureFlagRolloutRepository`.
- [ ] Data tab: export prompts path → file written; import prompts path → preflight → confirm → entities imported.
