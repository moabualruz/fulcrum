...
`src/theme/generator.ts`: reads all `tenant_settings(key LIKE 'theme.%')` rows for org + user; emits a CSS custom-property block (`--fulcrum-accent`, `--fulcrum-radius`, `--fulcrum-font-family`, `--fulcrum-spacing-unit`, `--fulcrum-animation-duration`, `--fulcrum-dark-mode`); user overrides win over org-wide values; defaults applied for missing keys; accent HEX validation; dark/light/auto → `prefers-color-scheme` media block. `src/theme/composable.ts`: `useTheme()` Svelte composable — SSR-safe (no `window` access during hydration); reactive on `tenant_settings` change via `theme.onSettingsChange` tRPC subscription. `theme.get`, `theme.update(key, value)`, `theme.reset` tRPC procedures with `assertPermission()`. CLI: `fulcrum theme list/get/set/reset [--json]`. TUI: reads theme at startup; applies accent to focused borders.

Cuts through: `theme.update('theme.accent', '#6D28D9')` → `tenant_settings` row → `generator.ts` rebuilds CSS block → `useTheme()` reactive update → `:root` vars updated in browser.
...
- [ ] `theme.update` persists to `tenant_settings`; `theme.get` returns same map; `theme.reset` restores all defaults.
- [ ] CLI `fulcrum theme set accent '#6D28D9'` → DB updated; `fulcrum theme get accent --json` → `{key:'theme.accent', value:'#6D28D9'}`.
...
- Issue 01 (schema) — `tenant_settings` table (from Pillar 1 base DDL) must exist.
- Pillar 1 issue 03 (composite indexes + flag stubs) — `tenant_settings` seeded.
