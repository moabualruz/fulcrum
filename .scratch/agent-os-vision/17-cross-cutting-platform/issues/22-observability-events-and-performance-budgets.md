...
- [ ] Every mutation tRPC procedure emits `Event` entity with correct `subject_kind` + `verb` + `payload` (no plaintext secrets).
...
- [ ] `TelemetryEvent` write < 2ms: 100 sequential writes; p99 assertion.
