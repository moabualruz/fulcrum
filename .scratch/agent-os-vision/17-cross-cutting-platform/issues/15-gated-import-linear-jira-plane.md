...
- [ ] All importers: `--dry-run` shows counts without repository writes; `--json` returns `{imported, skipped, errors}`.
- [ ] API key missing from `CredentialRepository` → error "Credential 'LINEAR_API_KEY' not found; run: fulcrum secrets set LINEAR_API_KEY".
...
- Issue 02 (secrets) — API keys read from `CredentialRepository`.
