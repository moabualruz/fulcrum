...
- [ ] `errorLogs.get(id)`: full entry including stack trace; `errorLogs.clear(before)` deletes rows + JSONL lines.
...
- [ ] Vitest: trigger uncaught exception in test harness → file written + DB row within 500ms; unhandled rejection same.
- [ ] Doctor: `platform.crashlog_dir` check verifies `~/.fulcrum/state/errors/` exists and writable.

## Blocked by

- Issue 01 (schema) — `error_logs` table must exist.
