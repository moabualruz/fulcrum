...
- [ ] Rust impl: cache-aside in `embed()` — `EmbedCacheEntry` hit returns stored blob without calling `fastembed-rs`; `hitCount` incremented; second call with identical text triggers zero model inference (verified by counter mock in test).
