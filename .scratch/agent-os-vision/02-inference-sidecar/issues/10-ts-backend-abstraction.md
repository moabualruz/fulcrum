...
- [ ] TS wrapper: `client.ts` reads `FULCRUM_INFERENCE_BACKEND` (global default) + feature-qualifier map (`embeddings:ollama` → Ollama for embed calls, `router-llm:embedded` → Embedded for generate calls); falls back to `embedded` when no env set.
