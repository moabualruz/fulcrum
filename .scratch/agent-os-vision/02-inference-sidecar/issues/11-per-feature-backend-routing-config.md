...
## Notes
- Routing map persisted via `isEnabled` / config store from Pillar 1 — no new table needed if Pillar 1's feature flag store supports value payloads. Otherwise add `inference_routing jsonb` column to `app_config` or dedicated table.
- Default map (shipped, no env override needed): `{ embeddings: 'embedded', 'router-llm': 'embedded', 'memory-llm-extract': 'embedded' }`.
- Config change does NOT require sidecar restart — `client.ts` reads map per-call.
