...
- `repomix --compress .` (or focused subdir) — when starting unfamiliar repo, user ask cross-cutting question. Instead of opening 30+ files sequentially. Why: each `Read` consumes context permanently; one packed digest cheaper. [source: https://code.claude.com/docs/en/best-practices]
