...
- **Coding standards: DDD, SOLID, DRY-lite.** Apply three principles, in priority order. **DDD** — model the domain in bounded contexts with `domain`/`application`/`infrastructure`/`interface` layers; the domain layer has zero infrastructure imports. **SOLID** — single responsibility per module/class/function; depend on abstractions not concretions; small focused interfaces. **DRY-lite** — remove genuine duplication of *knowledge* (one canonical home for a shared type/rule/algorithm), but **single responsibility and readability win over DRY**: do NOT abstract two things that merely look alike, do NOT add indirection/config/generics to delete a few duplicated lines, do NOT couple unrelated callers through a shared helper just to avoid repetition. Prefer a little duplication over the wrong abstraction. Why: over-applied DRY produces premature abstractions that couple unrelated code and obscure intent — the readable, locally-obvious version is better until the duplication is proven and identical in *meaning*.
