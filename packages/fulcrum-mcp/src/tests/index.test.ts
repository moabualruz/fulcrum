...
describe('fulcrum-mcp package', () => {
  ...
  it('package.json depends on @moabualruz/fulcrum-cli', async () => {
    const { readFileSync } = await import('fs')
    const pkgPath = resolve(REPO_ROOT, 'packages/fulcrum-mcp/package.json')
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf8')) as Record<string, unknown>
    const deps = pkg['dependencies'] as Record<string, string> | undefined
    expect(deps?.['@moabualruz/fulcrum-cli']).toBeDefined()
  })
  ...
  it('@moabualruz/fulcrum-memory package has kuzu as optionalDependency', async () => {
    const { readFileSync } = await import('fs')
...
describe('argv injection', () => {
  ...
  it('injects serve mcp at index 2', async () => {
    ...
    vi.doMock('@moabualruz/fulcrum-cli', () => ({}))
