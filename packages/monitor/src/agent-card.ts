...
//
// Delegates per-definition card building to buildA2ACard() in @moabualruz/fulcrum-core
// so both call-sites produce spec-compliant cards from a single builder.

import { getDb, listAgentDefinitions, buildA2ACard, A2A_PROTOCOL_VERSION, GLOBAL_WORKSPACE_ID, Db} from '@moabualruz/fulcrum-core'
import type { A2ASkill } from '@moabualruz/fulcrum-core'
...
export function buildAgentCard(options: {
  ...
  const db: Db = getDb()
  const defs = listAgentDefinitions(undefined, options.workspace_id ?? GLOBAL_WORKSPACE_ID, db)
