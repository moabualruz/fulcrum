...
export const TOOL_SCHEMAS: ToolSchema[] = [
  ...
  },
  {
    ...
    description: 'Memory v3 retrieval: FTS5 + vector + graph traversal fused via weighted RRF. Filtered by confidence floor + supersession. Returns L1 curated pages + L0 back-refs (sources[] + l0_wikilinks[]) — follow any claim to raw via `read_raw_source`. If no searchable L1 pages are seeded, returns reason=not_seeded with readiness instead of warming embeddings or running recall. `recall_memory` remains back-compat alias. workspace_id defaults cwd; project_id optional.',
    annotations: { readOnlyHint: true, openWorldHint: true },
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        text: { type: 'string', description: 'Natural-language or lexical code query' },
        workspace_id: { type: 'string', description: 'Workspace ID; defaults to current context' },
        project_id: { type: 'string', description: 'Project ID; defaults to current context when omitted by action handler' },
        symbol: { type: 'string', description: 'Symbol name or suffix to prefer' },
        lang: { type: 'string', description: 'Language filter, for example typescript or python' },
        path: { type: 'string', description: 'Relative path filter/hint' },
        package: { type: 'string', description: 'Package path/name hint, for example packages/memory' },
        module: { type: 'string', description: 'Module or path segment hint, for example retrieval' },
        dependency: { type: 'string', description: 'Imported dependency to prefer, for example better-sqlite3' },
        changed_files: { type: 'array', items: { type: 'string' }, description: 'Relative changed-file hints to boost' },
        scope: { type: 'string', enum: ['session', 'project', 'workspace', 'global'] },
        ...
        results: {
          ...
          items: {
            ...
            properties: {
              ...
              code_index_state: { type: 'string' },
              parse_status: { type: 'string', enum: ['parsed', 'skipped', 'failed'] },
              vector_status: { type: 'string', enum: ['pending', 'current', 'stale', 'failed', 'skipped', 'legacy'] },
              freshness: { type: 'string', enum: ['current', 'stale', 'failed', 'unknown'] },
              indexed_at: { type: 'string' },
              stage_scores: { type: 'object' },
              stage_contributions: { type: 'array', items: { type: 'object' } },
              runtime_truth: { type: 'object' },
              explanation: { type: 'object' },
            },
          },
        },
        reason: { type: 'string', enum: ['no_match', 'below_floor'] },
        skipped_stages: { type: 'array', items: { type: 'object' } },
      },
      required: ['results'],
    },
  },
  ...
  {
    title: 'Start RAG Repair',
    name: 'start_rag_repair',
    description: 'Execute targeted RAG repair through the rebuild pipeline, persisting a repair run, rebuild report, verification outcome, retry guidance, and audit event.',
    annotations: { destructiveHint: true },
    longRunningHint: true,
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    name: 'get_rag_health',
    description: 'Read-only RAG health report covering L0, L1, FTS, code, vector, embedding failure, stale state, graph coverage, degraded reasons, and explicitly out_of_scope domains. Does not persist health reports or mutate derived state.',
    annotations: { readOnlyHint: true, idempotentHint: true },
    inputSchema: {
      type: 'object',
      properties: {
        ...
        out_of_scope_domains: {
          type: 'array',
          items: { type: 'string' },
          description: 'Domains to mark out_of_scope for this read-only health check.',
        },
      },
    },
  },
  ...
  },
  {
    title: 'Run RAG Eval',
    name: 'run_rag_eval',
    description: 'Run a RAG eval suite and persist eval results. Model-heavy and accelerator-heavy cases stay opt-in. Returns terminal status or next_action for follow-up work.',
    ...
    inputSchema: {
      ...
      properties: {
        workspace_id: { type: 'string' },
        project_id: { type: 'string' },
        suite: { type: 'string', enum: ['rag-lifecycle', 'live-rag', 'code-rag', 'unified-context'] },
        include_model_heavy: { type: 'boolean' },
        include_accelerator_heavy: { type: 'boolean' },
        trigger_source: { type: 'string', enum: ['local', 'ci'] },
        trigger_scope: { type: 'string', enum: ['rag_related', 'non_rag', 'manual'] },
  ...
  },
  {
    ...
    description: 'Returns workspace_id + project_id for MCP-server cwd (deterministic, no file). Use at session start to discover workspace without reading .fulcrum.json. Also returns readiness: tools_available, monitor_url, monitor_running (200ms probe, 15s cache), suggested_next_call, and rag recall seed/degraded status. Read-only. Returns workspace_id, project_id, cwd, readiness.',
    annotations: { readOnlyHint: true, idempotentHint: true },
    inputSchema: {
      type: 'object',
      properties: {},
          ...
          properties: {
            tools_available: { type: 'number' },
	            ...
	            rag: {
	              type: 'object',
	              properties: {
	                recall_status: { type: 'string', enum: ['ready', 'degraded', 'not_seeded'] },
	                seeded: { type: 'boolean' },
	                searchable_rows: { type: 'number' },
	                degraded_stages: { type: 'array', items: { type: 'string' } },
	                next_actions: { type: 'array', items: { type: 'string' } },
	              },
	              required: ['recall_status', 'seeded', 'searchable_rows', 'degraded_stages', 'next_actions'],
	            },
	          },
	          required: ['tools_available', 'monitor_url', 'monitor_running', 'suggested_next_call', 'rag'],
	        },
      },
