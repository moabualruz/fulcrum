...
describe('full lifecycle integration', () => {
  ...
  it('agent run lifecycle: created → running → heartbeat → finished', async () => {
    ...
    const run = await startAgentRun({ context_type: 'primary', task_id: task.task_id, workspace_id: 'ws_1', role: 'software_engineer', agent_id: 'agent-001' })
  ...
  it('blocking a run sets blocker, status_category=blocked, emits event', async () => {
    ...
    const run = await startAgentRun({ context_type: 'primary', task_id: task.task_id, workspace_id: 'ws_1', role: 'code_reviewer', agent_id: 'agent-002' })
  ...
  it('listAgentProfiles returns 24 roles; workspace status reflects run counts', async () => {
    ...
    await startAgentRun({ context_type: 'primary', task_id: task.task_id, workspace_id: 'ws_1', role: 'qa_engineer', agent_id: 'a1' })
