...
export function runMigrations(db: Database.Database): void {
  ...
  const already009 = db.prepare("SELECT id FROM schema_migrations WHERE name = '009_monitor'").get()
  if (!already009) {
    db.exec(MIGRATION_009_MONITOR)
    db.prepare(`INSERT OR IGNORE INTO schema_migrations(name) VALUES ('009_monitor')`).run()
  }

  const already010 = db.prepare("SELECT id FROM schema_migrations WHERE name = '010_sync'").get()
  if (!already010) {
    db.exec(MIGRATION_010_SYNC)
    db.prepare(`INSERT OR IGNORE INTO schema_migrations(name) VALUES ('010_sync')`).run()
  }

  const already011 = db.prepare("SELECT id FROM schema_migrations WHERE name = '011_graph'").get()
  if (!already011) {
  ...
  const already018 = db.prepare("SELECT id FROM schema_migrations WHERE name = '018_memory_importance'").get()
  if (!already018) {
    try {
  ...
  const already019 = db.prepare("SELECT id FROM schema_migrations WHERE name = '019_agent_state_projection'").get()
  if (!already019) {
    db.exec(`
  ...
  const already020 = db.prepare("SELECT id FROM schema_migrations WHERE name = '020_round_1_gap_fixes'").get()
  if (!already020) {
  ...
  const already021 = db.prepare("SELECT id FROM schema_migrations WHERE name = '021_advisory_locks'").get()
  if (!already021) {
  ...
  const already022 = db.prepare("SELECT id FROM schema_migrations WHERE name = '022_handoff_mode_check'").get()
  if (already022) return
  ...
  const already023 = db.prepare("SELECT id FROM schema_migrations WHERE name = '023_memory_scope_task'").get()
  if (already023) return
  ...
  const already024 = db.prepare("SELECT id FROM schema_migrations WHERE name = '024_project_description'").get()
  if (already024) return
  ...
  const already025 = db.prepare("SELECT id FROM schema_migrations WHERE name = '025_tasks_and_runs_checks'").get()
  if (already025) return
  ...
  const already026 = db.prepare("SELECT id FROM schema_migrations WHERE name = '026_memory_kind_align'").get()
  if (already026) return
  ...
  const already027 = db.prepare("SELECT id FROM schema_migrations WHERE name = '027_add_missing_checks'").get()
  if (already027) return
  ...
  const already031 = db.prepare("SELECT id FROM schema_migrations WHERE name = '031_agent_definitions'").get()
  if (!already031) {
    db.exec(`
  ...
  const already032b = db.prepare("SELECT id FROM schema_migrations WHERE name = '032b_seed_agent_definitions'").get()
  if (!already032b) {
  ...
  const already034 = db.prepare("SELECT id FROM schema_migrations WHERE name = '034_missing_indices'").get()
  if (!already034) {
    db.exec(`
  ...
  const already035 = db.prepare("SELECT id FROM schema_migrations WHERE name = '035_composite_indexes'").get()
  if (!already035) {
    db.exec(`
  ...
  const already036 = db.prepare("SELECT id FROM schema_migrations WHERE name = '036_agent_definitions_workspace_scope'").get()
  if (!already036) {
