...
- User want run SQL query, script, or REPL against any of usql 30+ drivers.
- Agent need one-shot `SELECT` from CI or hook script with connection URL (pass positional or via `-f conn-url-file`).
- User jump between postgres, mysql, sqlite — want one tool, consistent flags, history.
- User ask "psql-like" experience against non-postgres DB.

**Skip** for: schema migrations (use `atlas`, `dbmate`, `golang-migrate`, `sqitch`); non-SQL stores (redis, kafka, mongo find — use native client); GraphQL/REST (use `xh` / `gh api`); bulk CSV import production-scale (use `pg_dump` / `COPY` / `mysqlimport` direct).
...
Schemes normalised: `postgres://`, `postgresql://`, `pg://` all work. Driver list: <https://github.com/xo/usql#database-support>.
...
Use `-f script.sql` when SQL has semicolons inside string literals — `-c` splits naive on `;`.

### Pattern C — output formats

`-o, --out FILE` is **output destination file**, NOT format selector. Format flags independent:
...
Long forms: `--json`, `--csv`, `--html`, `--expanded`, `--vertical`, `--no-align`, `--tuples-only`. Combine with `-o results.json` to write file instead stdout. JSON pair natural with `jq` skill.
...
Same `\c` semantics as psql; prompt update to reflect active driver.
...
`:NAME` is literal substitution (psql-compatible). For untrusted input prefer driver-side prepared statements via real client library, not usql.
...
Some commands driver-specific — `\d` work against postgres / mysql / sqlite, but coverage thin for snowflake / bigquery. Fall back to driver-native SQL (`SHOW TABLES`, `INFORMATION_SCHEMA.TABLES`) when `\d` error.

## Anti-patterns

- **Don't put credentials in argv.** `usql 'pg://user:pass@host/db'` show up in `ps`, shell history, CI logs. Read DSN from file (`usql "$(cat conn-url)" ...`), `.pgpass`-style file, or `~/.config/usql/config.yaml` named connections (`usql my-db`).
- **Don't rely on cross-driver SQL.** usql normalise *connection* not *dialect* — `SHOW DATABASES` is mysql, `\l` list databases on postgres, `LIMIT` syntax differ on mssql/oracle. Test SQL against target engine.
...
- **Don't trust `\copy` to behave identically across drivers.** psql concept partially emulated for mysql/sqlite/etc.; quoting, NULL handling, header detection differ. For large or repeatable loads use driver native bulk path (`COPY`, `LOAD DATA INFILE`, `bcp`).
- **Don't pass multi-statement input to `-c` when statements contain semicolons in string literals.** Split is naive (`strings.Split(input, ";")`), so `-c "INSERT INTO t VALUES ('a;b')"` corrupts. Use `-f script.sql`.
- **Don't assume connection URLs interchangeable across drivers.** `sslmode=require` is postgres; mysql use `tls=true`. Ports default differently (5432 / 3306 / 1433). Encoding params (`charset`, `client_encoding`) driver-specific.
- **Don't pipe default aligned output into another tool.** ASCII borders break parsers. Use `-C` (CSV) or `-J` (JSON) for downstream pipelines. Remember: `-o` write to file; format selected separately.
- **Don't reach for usql when only one engine in play and native client already installed.** `psql`, `sqlite3`, `duckdb` have richer driver-specific features (psql `\watch`, sqlite `.dump`); usql value is multi-driver shape.

## Cross-refs

- Behavioral rule: see `rules/AGENTS.md` — data-access section ("never hard-code credentials in argv; prefer `$DATABASE_URL`").
- JSON output: `skills/jq/SKILL.md` — `usql -J | jq` is canonical agent shape.
