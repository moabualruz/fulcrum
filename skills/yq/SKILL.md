...
yq --doc 0 '.metadata.name' manifests.yaml
...
yq -V cluster=prod '.config.cluster = strenv(cluster)' f.yaml   # NOTE: -V binds string vars
yq --argjson port 8080 '.service.port = $port' f.yaml           # JSON-typed
...
- **Don't path into multi-document YAML without `select` or `--doc N`.** `yq '.kind' manifests.yaml` emits one value per document, which surprises pipelines that expect a single answer. Filter explicitly.
...
- **Don't shell-interpolate values into the expression.** `yq ".tag = \"$VAR\""` breaks on quotes/spaces and is a quoting hazard. Use `-V name=value` (string, then `strenv(name)`) or `--argjson name '<json>'` (typed).
