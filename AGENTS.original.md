...
- **Reuse-first product engineering.** Before implementing any non-trivial product or platform feature, research existing free/open-source tools, libraries, schemas, UI blocks, workflow engines, CLIs, and self-hostable apps that can cover it. Prefer dependencies and embeddable/local/self-hosted building blocks over bespoke code; hosted third-party integrations are acceptable when they materially shorten the path without compromising local-first defaults. If a candidate covers roughly 75% or more of the required behavior with acceptable license, runtime, and ownership risk, adopt it and build the remaining gap. If fit is below roughly 75%, strategic, or unclear, stop and present options for the user to choose before building.
- **Documentation retrieval is deterministic by default.** For Fulcrum's project-management, documentation, memory, and context surfaces, do not introduce embeddings, RAG pipelines, semantic search, or local/remote model dependencies unless the user explicitly approves that design. Prefer structured metadata, full-text search, filters, backlinks, source references, task/doc relationships, and deterministic query/index engines.
