...
12. **Android safe areas.** Web shell uses `viewport-fit=cover` and shared safe-area tokens. Portrait mobile reserves at least 24 px above chrome for the Android status bar and at least 48 px below bottom navigation for the gesture zone. Landscape mobile reserves 48 px on both inline edges and keeps a 16 px bottom affordance. Design E2E validates this via `/cross-cutting-mobile`; production routes must not lower these reserves to match incomplete mockups.
