...
export class Migration20260502030300_agent_runs_symphony_columns extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `alter table "agent_runs" add constraint "agent_runs_task_id_foreign" foreign key ("task_id") references "tasks" ("id")`,
    );
    this.addSql(
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `alter table "agent_runs" drop constraint if exists "agent_runs_task_id_foreign"`,
    );
