...
export class Migration20260502070200_docs_related_tables extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `alter table "doc_links" add constraint "doc_links_from_doc_id_foreign" foreign key ("from_doc_id") references "documents" ("id") on delete cascade`,
    );
    this.addSql(
      `alter table "doc_links" add constraint "doc_links_to_doc_id_foreign" foreign key ("to_doc_id") references "documents" ("id") on delete set null`,
    );
    ...
    this.addSql(
      `alter table "doc_versions" add constraint "doc_versions_doc_id_foreign" foreign key ("doc_id") references "documents" ("id") on delete cascade`,
    );
    this.addSql(
      `alter table "doc_versions" add constraint "doc_versions_author_id_foreign" foreign key ("author_id") references "users" ("id") on delete set null`,
    );
    this.addSql(
      `alter table "doc_versions" add constraint "doc_versions_restore_of_foreign" foreign key ("restore_of") references "doc_versions" ("id") on delete set null`,
    );

    this.addSql(
      `create table "doc_comments" ("id" uuid not null default gen_random_uuid(), "org_id" uuid not null, "doc_id" uuid not null, "anchor_range" jsonb null, "author_id" uuid not null, "body_md" text not null, "parent_comment_id" uuid null, "resolved" boolean not null default false, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), primary key ("id"))`,
    );
    this.addSql(
    ...
    this.addSql(
      `alter table "doc_comments" add constraint "doc_comments_doc_id_foreign" foreign key ("doc_id") references "documents" ("id") on delete cascade`,
    );
    this.addSql(
      `alter table "doc_comments" add constraint "doc_comments_author_id_foreign" foreign key ("author_id") references "users" ("id")`,
    );
    this.addSql(
      `alter table "doc_comments" add constraint "doc_comments_parent_comment_id_foreign" foreign key ("parent_comment_id") references "doc_comments" ("id") on delete cascade`,
    );
