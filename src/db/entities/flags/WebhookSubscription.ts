 ...
 * `outbound-webhooks` feature flag is enabled. Pillar 10 dispatches HTTP
 * POSTs against `url` for events whose `verb` is in the `events` array.
 *
...
export class WebhookSubscription {
  ...
  /** Outbound URL — must be https in production (validated by Pillar 10). */
  @Property({ type: "string" })
  url!: string;

  /** Event verbs this subscription cares about (e.g. ["task.created"]). */
  @Property({ type: "json" })
  events!: string[];
