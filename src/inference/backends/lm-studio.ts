/**
 * LmStudioBackend — OpenAI-compat at localhost:1234/v1.
 */
import type {
  InferenceBackend,
  EmbedRequest,
...
export class LmStudioBackend implements InferenceBackend {
  ...
  async tokenize(req: TokenizeRequest): Promise<TokenizeResponse> {
    // LM Studio has no tokenize endpoint; estimate
  ...
  async health(): Promise<HealthResult> {
    try {
      ...
      return { backend: "lm-studio", status: "ok", models };
    } catch (err) {
      return {
        backend: "lm-studio",
        status: "down",
  ...
  private async post(path: string, body: unknown): Promise<any> {
    ...
    if (!res.ok) throw new Error(`LM Studio ${path}: HTTP ${res.status}`);
    return res.json();
  }
}
