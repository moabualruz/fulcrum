...
import { DocService } from "./DocService.ts";
...
async function ensureUser(em: TestOrm["em"], id = USER_ID, role = "member"): Promise<void> {
...
async function createProject(em: TestOrm["em"], name = "DocService class project"): Promise<string> {
  const id = randomUUID();
...
describe("DocService class with real MikroORM persistence", () => {
  test("creates parent and child docs, filters lists, updates body/content, and deletes softly/hard", async () => {
    ...
    await ensureUser(em);
    const service = new DocService(em);
    ...
    await expect(service.get(DEFAULT_ORG_ID, { id: child.id })).rejects.toMatchObject({ code: "NOT_FOUND" });
  ...
  test("creates comment threads, sorts by anchor, resolves roots, updates/deletes with author/admin checks", async () => {
    ...
    await expect(service.resolveComment(ctx, reply.id, true)).rejects.toMatchObject({ code: "BAD_REQUEST" });

    const stranger = { orgId: DEFAULT_ORG_ID, userId: randomUUID(), em };
    await expect(service.updateComment(stranger, early.id, "not allowed")).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(service.deleteComment(stranger, late.id)).rejects.toMatchObject({ code: "FORBIDDEN" });
  ...
  test("lists, diffs, restores versions and reports forward/back links from wikilinks", async () => {
    ...
    await expect(service.listVersions(ctx, randomUUID())).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(service.diffVersions(ctx, randomUUID(), 1, 2)).rejects.toMatchObject({ code: "NOT_FOUND" });
  });
