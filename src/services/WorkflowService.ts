/**
 * WorkflowService — Phase 05 Plan 04.
 *
...
import { Project, type WorkflowConfig } from "../db/entities/tasks/Project.ts";
...
export class WorkflowService {
  ...
  private async findProject(orgId: string, projectId: string): Promise<Project> {
    ...
    if (!project) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: `Project ${projectId} not found in org ${orgId}`,
      });
    }
  ...
  async updateEnabledTaskTypes(
    ...
    if (invalid.length > 0) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `Invalid task types: ${invalid.join(", ")}. Valid types: ${TASK_TYPES.join(", ")}`,
      });
    }
