...
import { TaskService, normalizedUnique } from "./TaskService.ts";
...
async function createProject(em: TestOrm["em"], name = "TaskService class project"): Promise<string> {
  const id = randomUUID();
  await em.getConnection().execute(
    `INSERT INTO projects (id, org_id, slug, name, description, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, now(), now())`,
    [id, DEFAULT_ORG_ID, `task-service-class-${id.slice(0, 8)}`, name, "TaskService class integration"],
  );
  return id;
}

describe("TaskService class with real MikroORM persistence", () => {
