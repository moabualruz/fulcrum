...
export class TaskService {
  ...
  async setDependencies(
    ...
    if (referencedIds.size !== dependencies.blocks.length + dependencies.blocked_by.length) {
      throw new TRPCError({ code: "CONFLICT", message: "Task dependency cycle rejected." });
    }
...
async function findBulkTasksOrThrow(repo: TaskRepository, orgId: string, ids: string[]): Promise<Task[]> {
  ...
  if (tasks.length !== ids.length) {
    throw new TRPCError({ code: "NOT_FOUND", message: "One or more tasks were not found." });
  }
  return tasks;
}
...
async function assertParentDoesNotCycle(input: {
  ...
  while (cursor) {
    if (cursor === input.taskId || seen.has(cursor)) {
      throw new TRPCError({ code: "CONFLICT", message: "Task parent cycle rejected." });
    }
    seen.add(cursor);
