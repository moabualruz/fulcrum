/**
 * RecurrenceService — Phase 05 Plan 04 (D-116).
 *
 * Manages TaskRecurrenceRule entities.
 * processDue() is called by graphile-worker; clones source task via TaskService.
...
import { TaskRecurrenceRule } from "../db/entities/tasks/TaskRecurrenceRule.ts";
...
export class RecurrenceService {
  ...
  async create(
    ...
    if (config.triggerType === "schedule" && !config.cronExpression && !config.intervalDays) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "Schedule recurrence requires cronExpression or intervalDays",
      });
    }
    ...
    if (!task) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: `Task ${sourceTaskId} not found`,
      });
    }
  ...
  async delete(orgId: string, ruleId: string): Promise<void> {
    ...
    if (!rule) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: `Recurrence rule ${ruleId} not found`,
      });
    }
