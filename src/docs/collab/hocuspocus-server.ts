/**
 * Hocuspocus server factory — P7#21 gated real-time collab.
 *
 ...
 * Shared server, different room prefixes (PRD §gated features).
 *
...
/** Default Hocuspocus port per PRD. */
...
export class HocuspocusPersistenceAdapter {
   ...
   * `task:` rooms are handled by Pillar 6 (not this adapter).
   */
  extractDocId(roomName: string): string | null {
    if (roomName.startsWith("doc:")) {
      return roomName.slice(4);
    }
    // task: rooms handled by Pillar 6, not docs persistence
    return null;
  }
}
