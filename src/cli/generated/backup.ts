...
export function createBackupCommand(): Command {
  ...
  createCommand.action(async (options) => {
    try {
      throw new Error("Generated tRPC invocation for backup.create is not wired yet.");
    } catch (error) {
  ...
  restoreCommand.action(async (options) => {
    try {
      throw new Error("Generated tRPC invocation for backup.restore is not wired yet.");
    } catch (error) {
      if (options.json === true) {
        const message = error instanceof Error ? error.message : String(error);
        console.log(JSON.stringify({ error: { code: "INTERNAL_ERROR", message } }));
