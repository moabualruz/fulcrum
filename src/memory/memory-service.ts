/**
 * MemoryService — Plan 06-06
 *
 * Provides CRUD + FTS search with project>global ranking and promotion.
 *
 * D-24: project-scoped memories rank above global for same-project queries.
 * D-27: promote() sets global=true, preserves original projectId for audit trail.
 * T-06-13: org_id filter on all queries — no cross-tenant leak.
 * T-06-14: promotion guarded by orgId ownership check.
 */

import { injectable as Injectable, inject } from "@needle-di/core";
...
// Importance multipliers per D-24 spec
...
export class MemoryService {
  constructor(
    private readonly repo = inject(MemoryRepository),
  ) {}
   ...
   *
   * D-24: ORDER BY (project_id = $projectId)::int DESC, weighted_rank DESC
   */
   ...
   * Per D-27: sets global=true, preserves projectId for audit trail.
   * T-06-14: orgId guard ensures only org members can promote.
   */
   ...
   * T-06-13: orgId filter ensures no cross-tenant leak.
   */
