/**
 * Filter AST for saved views and search (Pillar 6 + Pillar 11).
 *
 * Q27: Search reuses this same AST — `kind` field discriminates
 * task/doc/memory/artifact. This module lives in `src/filters/` so both
 * pillars can import it without circular dependency.
 *
 * C6: No raw SQL. compileSavedViewQuery returns a MikroORM FilterQuery
 * expression; the ORM generates the SQL.
 */
...
export const FacetsSchema = z
  .object({
    /** Q27: discriminates task/doc/memory/artifact in unified search */
...
export const SavedViewQuerySchema = z.object({
  ...
  /** FTS fragment — always-on LIKE path; Pillar 11 swaps to tsvector. */
 ...
 * Pillar 11 swap: replace with tsvector-based `$raw` once full-text index lands.
 *
 * `custom_fields.<slug>` uses jsonb `$contains` (`@>` operator).
 * Native `->>'slug'` path ops need Pillar 11 MikroORM jsonb extension or $raw.
 */
export function compileSavedViewQuery(q: SavedViewQuery): FilterQuery<Task> {
  const conditions: Record<string, unknown>[] = [];

  // FTS text fallback — title LIKE (Pillar 11 replaces with tsvector)
  if (q.text) {
...
function compileClause(clause: FilterClause): Record<string, unknown> | null {
  ...
  // Native custom_fields->>'slug' path needs Pillar 11 jsonb extension or $raw.
...
function compileCustomFieldOp(
  ...
  switch (op) {
    ...
    default:
      // gt/lt/nin/contains on jsonb path need native ops (Pillar 11)
      return { $contains: { [slug]: value } };
  }
}
