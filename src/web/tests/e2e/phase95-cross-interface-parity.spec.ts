...
test.describe("Phase 09.5 web cross-interface parity", () => {
  test("application-created task data is visible and peer web routes fail closed without skips", async ({ page, fulcrumHome }) => {
    const project = await fulcrumHome.seedProject("phase95-cross-interface", "Phase 95 Cross Interface");
    const artifact = await fulcrumHome.seedArtifact({
      title: "phase95-cross-interface-artifact.txt",
      mime: "text/plain",
    });
    const searchPhrase = "phase95-web-search-parity";

    await expectOkPage(page, "/boards");
    await page.locator("[data-board-column-input]").first().fill("Phase 95 Cross Interface Task");
    await page.locator("[data-board-column-input]").first().press("Enter");
    await expect(page.locator("body")).toContainText("Phase 95 Cross Interface Task");
