...
export async function updateTaskAction(
  ...
  if (changed.includes("due_date") || changed.includes("start_date")) {
    await conn.execute(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS due_date date`);
    await conn.execute(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS start_date date`);
  }
