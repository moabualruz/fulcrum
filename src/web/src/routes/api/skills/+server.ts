  ...
  try {
    ...
    switch (action) {
      case "install": {
        ...
        try {
          const skill = await installSkill(db, { orgId, slug, upstreamRepo });
      ...
      case "upgrade": {
        ...
        try {
          if (slug === "all") {
            const skills = await upgradeAllSkills(db, orgId);
            return jsonOk(skills);
          }
          const skill = await upgradeSkill(db, orgId, slug);
          return jsonOk(skill);
        } catch (e) {
      ...
      case "uninstall": {
        ...
        if (!slug) return jsonError("slug is required");
        try {
          await uninstallSkill(db, orgId, slug);
      ...
      case "update_enabled_agents": {
        ...
        try {
          const skill = await updateEnabledAgents(db, orgId, slug, agents);
          return jsonOk(skill);
        } catch (e) {
      ...
      case "resolve_conflict": {
        ...
        try {
          const skill = await resolveConflict(db, { orgId, slug, resolution });
          return jsonOk(skill);
        } catch (e) {
