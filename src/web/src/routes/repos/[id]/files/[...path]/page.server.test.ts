...
describe("/repos/[id]/files/[...path] +page.server.ts", () => {
  test("load returns file content for a text file", async () => {
    ...
    try {
      await insertRepoFile(db, {
        repoId, branch: "main", path: "src/index.ts", kind: "file",
        parentPath: "src", mime: "text/typescript",
      });
      await upsertFileContent(db, {
        repoId, branch: "main", path: "src/index.ts",
    ...
    const result = await mod.load({
      params: { id: repoId, path: "src/index.ts" },
      url: new URL(`http://localhost/repos/${repoId}/files/src/index.ts`),
  ...
  test("load returns blame when ?blame=1", async () => {
    ...
    const result = await mod.load({
      params: { id: repoId, path: "src/index.ts" },
      url: new URL(`http://localhost/repos/${repoId}/files/src/index.ts?blame=1`),
    } as Parameters<typeof mod.load>[0]);
