/**
 * metrics-rollup worker — Plan 05-05.
 *
 ...
 *
 * Debounce: T-05-12 mitigation — one rollup per scope per invocation
 * (caller responsible for dedup / 5s debounce before enqueue).
 *
 * C6: No raw SQL — MikroORM only.
 * T-05-11: All queries scoped by orgId.
 */
...
async function computeSnapshot(
  ...
  // Build task query filter scoped by orgId (T-05-11)
...
async function handleMetricsRollup(
  ...
  // Upsert MetricsCache row for today + scope (T-05-12: idempotent per scope per day)
 ...
 *
 * T-05-12 mitigation: dedup logic should wrap enqueue call in caller.
 */
