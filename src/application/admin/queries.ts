...
import type { InferenceModel } from "../../inference/protocol.ts";
...
export function decodeBackupDump(encoded: string): BackupDump {
  try {
    ...
    throw new TRPCError({ code: "BAD_REQUEST", message: "Backup dump is invalid.", cause });
  }
}
...
export function normalizeThemeKey(key: string): ThemeKey {
  ...
  throw new TRPCError({ code: "BAD_REQUEST", message: `Unknown theme key '${key}'.` });
}

export function validateThemeValue(key: ThemeKey, value: string): string {
  const trimmed = value.trim();
  if (key === "theme.accent" && !/^#[0-9a-fA-F]{6}$/.test(trimmed)) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "theme.accent must be a 6-digit HEX color." });
  }
  if (key === "theme.dark-mode" && !["light", "dark", "auto"].includes(trimmed)) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "theme.dark-mode must be light, dark, or auto." });
  }
  return trimmed;
}
...
function themeRepo(ctx: AdminAppContext): ThemeSettingsRepository {
  if (!ctx.container?.has(ThemeSettingsRepository)) {
    throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Theme settings repository is not configured." });
  }
...
async function getFeatureFlagClass() {
  const { FeatureFlag } = await import("../../db/entities/auth/FeatureFlag.ts");
  return FeatureFlag;
}

async function getFeatureFlagRepository() {
  const { FeatureFlagRepository } = await import("../../db/repositories/auth/FeatureFlagRepository.ts");
  return FeatureFlagRepository;
}

async function getOrgMemberRepository() {
  const { OrgMemberRepository } = await import("../../db/repositories/auth/OrgMemberRepository.ts");
...
async function getFeatureFlagRolloutClass() {
  const { FeatureFlagRollout } = await import("../../db/entities/platform/FeatureFlagRollout.ts");
...
async function requireOwnerOrAdmin(ctx: AdminAppContext): Promise<void> {
  ...
  if (!membership || (membership.role !== "owner" && membership.role !== "admin")) {
    throw new TRPCError({ code: "FORBIDDEN", message: "Only org owners and admins can modify feature flags." });
  }
}
...
async function requireWritableFlagScope(ctx: AdminAppContext, input: { orgId?: string; userId?: string }): Promise<string> {
  if (input.orgId && input.orgId !== ctx.orgId) {
    throw new TRPCError({ code: "FORBIDDEN", message: "Cannot modify feature flags outside the active org." });
  }
  if (input.userId) {
    const targetMembership = await findOrgMembership(ctx, ctx.orgId, input.userId);
    if (!targetMembership) {
      throw new TRPCError({ code: "FORBIDDEN", message: "Cannot modify feature flags for users outside the active org." });
    }
  }
  return ctx.orgId;
}
...
export async function evaluateFlag(ctx: AdminAppContext, input: { flag: FeatureFlagName; orgId: string; userId: string }) {
  if (input.orgId !== ctx.orgId) {
    throw new TRPCError({ code: "FORBIDDEN", message: "Cannot evaluate feature flags outside the active org." });
  }
  const registry = await resolveFlagRegistry(ctx);
