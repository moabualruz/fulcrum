...
import { requireAdminOrOwner, requireOwner, type OrgAppContext } from "./queries.ts";
...
export async function updateOrgMemberRole(
  ...
  await requireOwner(em, ctx);
  const member = await em.findOne(OrgMember, { orgId: ctx.orgId, userId: input.userId } as never);
