import { fulcrumHome, openDatabase } from "../../config/database.ts";
...
async function withDb<T>(fn: (db: SqlExecutor, orgId: string) => Promise<T>): Promise<T> {
  ...
  try {
    let org = await defaultOrg(db);
    if (!org) {
      await db.close();
      db = await openDatabase({ backend: "pglite", dataDir: join(fulcrumHome(), "pglite.data", "main") });
      org = await defaultOrg(db);
    }
