...
const CHECKPOINTS: UatCheckpoint[] = [
  {
    ...
    evidence: [
      { path: "src/repos/__tests__/watcher.sla.test.ts", patterns: [/inside 2s/, /coalesces repeated bursts/, /repo\.sync\.local/] },
      { path: "src/repos/watcher.ts", patterns: [/debounceMs/, /repo\.sync\.local/] },
    ],
  },
  {
    number: 2,
    ...
    layers: ["unit"],
    evidence: [
      { path: "src/repos/__tests__/watcher.sla.test.ts", patterns: [/top 5 remote repos/, /enqueues no more than 5 remotes/] },
      { path: "src/repos/workers/sync-remote.ts", patterns: [/repo\.lru\.warmup/, /listRecentlyTouchedRemote\(REPO_LRU_WARMUP_LIMIT\)/] },
    ],
  },
  {
    number: 3,
    ...
    layers: ["unit", "integration"],
    evidence: [
      { path: "src/repos/__tests__/dashboard.test.ts", patterns: [/required fields/, /lastSyncError/, /watcherStatus/] },
      { path: "src/repos/dashboard.ts", patterns: [/openTaskCount/, /syncLatencyMs/, /lastSyncError/] },
    ],
  },
  {
    number: 4,
  ...
  },
  {
    ...
    layers: ["unit"],
    evidence: [
      { path: "src/artifacts/__tests__/pruner.test.ts", patterns: [/defaults project artifacts to forever/, /scratch artifacts to 90 days/] },
      { path: "src/db/entities/artifacts/ArtifactRetentionPolicy.ts", patterns: [/retentionDays/, /keepLatestPerRef/, /keepPinned/] },
    ],
  },
  {
    number: 8,
    ...
    layers: ["unit", "integration"],
    evidence: [
      { path: "src/artifacts/__tests__/pruner.test.ts", patterns: [/idempotent/, /pinned/, /org mismatch/] },
      { path: "tests/artifacts/pruner.test.ts", patterns: [/soft-archives expired artifacts/, /requires confirmation/] },
    ],
  },
  {
    number: 9,
    ...
    layers: ["unit", "integration"],
    evidence: [
      { path: "src/artifacts/__tests__/harvest-search.test.ts", patterns: [/run edge/, /searchable provenance/] },
      { path: "src/search/indexers/artifact.ts", patterns: [/SearchDocument/, /sourcePath/, /producerKind/] },
    ],
  },
  {
    number: 10,
  ...
  },
  {
    ...
    layers: ["unit", "integration"],
    evidence: [
      { path: "src/notifications/__tests__/fanout.test.ts", patterns: [/repo sync completed/, /artifact\.created/, /disabled rule/, /mute/] },
      { path: "tests/notifications/fanout-worker.test.ts", patterns: [/dedups in-app notifications/, /all four default rules/] },
    ],
  },
  {
    number: 12,
    ...
    layers: ["unit", "integration"],
    evidence: [
      { path: "src/notifications/__tests__/delivery-worker.test.ts", patterns: [/SMTP handler/, /webhook handler/, /push handler/] },
      { path: "src/notifications/delivery-handlers/push.ts", patterns: [/missing_config/] },
    ],
  },
  {
    number: 13,
    ...
    layers: ["unit", "integration"],
    evidence: [
      { path: "src/notifications/__tests__/delivery-worker.test.ts", patterns: [/X-Fulcrum-Signature|FULCRUM_SIGNATURE_HEADER/, /retry metadata/] },
      { path: "tests/webhooks/dispatcher.test.ts", patterns: [/HMAC-SHA256/, /exponential backoff/] },
    ],
  },
  {
    number: 14,
    ...
    layers: ["unit"],
    evidence: [
      { path: "src/notifications/__tests__/delivery-worker.test.ts", patterns: [/quiet-hours delivery holds/, /held-quiet-hours/] },
      { path: "src/notifications/quiet-hours.ts", patterns: [/nextAttemptAt/, /held-quiet-hours/] },
    ],
  },
  {
    number: 15,
  ...
  },
  {
    ...
    layers: ["unit", "integration"],
    evidence: [
      { path: "src/artifacts/__tests__/phase07-security.test.ts", patterns: [/cross-org download/, /traversal paths/, /hard confirmed/] },
      { path: "src/artifacts/storage.ts", patterns: [/isSubPath/, /deleteArtifact/] },
    ],
  },
  {
    number: 19,
...
describe("Phase 07 UAT automation coverage", () => {
  test("UAT file has one automatable checkpoint for every Phase 7 acceptance behavior", () => {
