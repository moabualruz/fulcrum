import { normalizeEditPermission } from "@planning-review/application/plan-mode.ts";

export type WorkflowMode = "manual" | "user-managed" | "plan-agent" | "all-agents";

export interface PlanningWorkflowRuntimeOptions {
  workflow?: unknown;
...
export function normalizeWorkflowOptions(rawOptions: PlanningWorkflowRuntimeOptions | null | undefined): NormalizedWorkflowOptions {
  const rawWorkflow = typeof rawOptions?.workflow === "string" ? rawOptions.workflow.trim() : "";
  const workflow = WORKFLOWS.has(rawWorkflow as WorkflowMode) ? (rawWorkflow as WorkflowMode) : DEFAULT_WORKFLOW;
...
export function isPlanningAgent(agentName: string | undefined, options: NormalizedWorkflowOptions): boolean {
  return Boolean(agentName && options.planningAgentSet.has(agentName));
}
...
export function shouldInjectFullPlanningPrompt(agentName: string | undefined, options: NormalizedWorkflowOptions): boolean {
...
export function shouldRejectSubmitPlanForAgent(agentName: string | undefined, options: NormalizedWorkflowOptions): boolean {
  return options.workflow === "plan-agent" && !isPlanningAgent(agentName, options);
}

export function applyWorkflowConfig(opencodeConfig: OpenCodeConfig, options: NormalizedWorkflowOptions, allowSubagents: boolean): void {
...
function resolveAgentConfigKey(opencodeConfig: OpenCodeConfig, agentName: string): string {
  ...
  for (const existingKey of Object.keys(opencodeConfig.agent)) {
    ...
    if (normalizedExisting === normalizedTarget) return existingKey;
