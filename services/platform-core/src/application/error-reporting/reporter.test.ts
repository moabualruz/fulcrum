// @ts-nocheck
/**
 * Gated remote error reporting tests.
 *
 * Tests:
 *   1. Flag OFF → no POST.
...
} from "@platform-core/application/error-reporting/reporter.ts";
...
describe("scrubPaths", () => {
  ...
  test("replaces Windows C:\\Users\\<name>\\... with <homedir>\\...", () => {
    const result = scrubPaths("C:\\Users\\alice\\Projects\\app.ts");
    expect(result).toBe("<homedir>\\Projects\\app.ts");
  });

  test("replaces multiple occurrences in a stack trace", () => {
    const stack = [
      "Error: boom",
  ...
  test("leaves non-path strings unchanged", () => {
...
describe("buildReportPayload", () => {
  test("includes scrubbed stack trace", () => {
  ...
  test("excludes PII fields (email, secret values)", () => {
    const entry: ErrorReportEntry = {
      id: "x",
      errorMessage: "fail",
      occurredAt: new Date(),
  ...
  test("does not include user_id or org_id in payload", () => {
    const entry: ErrorReportEntry = {
      id: "x",
      errorMessage: "fail",
      occurredAt: new Date(),
...
describe("signPayload / verifySignature", () => {
  ...
  test("produces a non-empty hex signature", () => {
...
describe("enqueueErrorReport", () => {
  ...
  test("flag OFF: does not enqueue any job", async () => {
    const enqueue = mock();
  ...
  test("flag ON: enqueues errors:report job with scrubbed payload", async () => {
    const enqueue = mock(() => Promise.resolve(undefined));
    const opts: EnqueueErrorReportOptions = {
      featureEnabled: true,
      endpoint: "https://example.com/errors",
      signingSecret: "secret",
  ...
  test("flag ON: job payload includes valid HMAC signature", async () => {
    const enqueue = mock(() => Promise.resolve(undefined));
...
describe("error reporting doctor check via platform checks", () => {
  test("flag OFF: platform.error_reporting status is skip", async () => {
    const { runPlatformDoctorChecks } = await import("@platform-core/application/platform-operations/readiness-checks.ts");
  ...
  test("flag ON + endpoint reachable: platform.error_reporting is pass", async () => {
    const { runPlatformDoctorChecks } = await import("@platform-core/application/platform-operations/readiness-checks.ts");
    const results = await runPlatformDoctorChecks({
      errorReporting: {
        featureEnabled: true,
  ...
  test("flag ON + 4xx response: platform.error_reporting is degraded (warn)", async () => {
    const { runPlatformDoctorChecks } = await import("@platform-core/application/platform-operations/readiness-checks.ts");
    const results = await runPlatformDoctorChecks({
      errorReporting: {
        featureEnabled: true,
  ...
  test("flag ON + no endpoint: platform.error_reporting is fail", async () => {
    const { runPlatformDoctorChecks } = await import("@platform-core/application/platform-operations/readiness-checks.ts");
    const results = await runPlatformDoctorChecks({
      errorReporting: {
        featureEnabled: true,
