/**
 * vault-adapter.ts — per-provider secret adapter interface.
 *
 * Gated behind FULCRUM_FEATURES=vault-integration. Online providers remain
 * default OFF.
 *
 * Providers:
...
export class LocalNaclProvider implements SecretProvider {
   ...
   * with how the credentials transport adapter handles the local path.
   *
   * NOTE: The credentials transport adapter does the actual DB lookup
  ...
  async get(name: string): Promise<string> {
    ...
    // purposes. Direct decryption belongs to the credentials transport adapter which has
    // the DB context. This implementation surfaces the provider identity.
    throw new Error(
      `LocalNaclProvider.get('${name}') — use credentials transport adapter for DB-backed decryption.`,
    );
  }

  async set(_name: string, _value: string): Promise<void> {
    throw new Error(
      "LocalNaclProvider.set — use credentials transport adapter for DB-backed encryption.",
    );
  }
}
