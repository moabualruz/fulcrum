/**
 * doctor-checks — secrets domain doctor integration.
 *
 * Native unavailable reports `keyring: degraded` (warn), never `fail`.
 * Native present reports pass.
 *
 * vaultHealthCheck: when vault-integration flag ON and Vault credentials are
 * configured, checks reachability. Vault unreachable with
 * vault credentials present → `fail` with recovery hint.
 *
 * The doctor aggregator imports `keyringHealthCheck` and registers it the same
 * way db/doctor-checks.ts is registered.
 */
...
/**
 * vaultHealthCheck — checks platform.keyring.vault.
 *
...
// ---------------------------------------------------------------------------
// platform.keyring + platform.keyring_mode checks
// ---------------------------------------------------------------------------
