...
async function buildServer(): Promise<string> {
  const proc = Bun.spawn(["cargo", "build", "--manifest-path", "services/inference-runtime/Cargo.toml", "-p", "inference-server"], {
    cwd: process.cwd(),
    stdout: "pipe",
    stderr: "pipe",
  });
  ...
  return join(process.cwd(), "services", "inference-runtime", "target", "debug", "inference-server");
}
