...
export async function listRepos(em: EntityManager, ctx: AppContext): Promise<RepoDto[]> {
  return (await em.find(Repo, { where: { org: { id: ctx.orgId }, archived: false } as never, order: { slug: "ASC" } })).map(serializeRepo);
}
...
export async function listRepoTree(em: EntityManager, ctx: AppContext, input: { repoId: string; commitSha: string }): Promise<RepoTreeEntryDto[]> {
  ...
  return (await em.find(RepoTreeEntry, { where: { org: { id: ctx.orgId }, repo: { id: input.repoId }, commitSha: input.commitSha } as never, order: { path: "ASC" } })).map(serializeRepoTreeEntry);
}
...
export async function listRepoPageRows(em: EntityManager, ctx: AppContext): Promise<RepoListRow[]> {
  ...
  const repos = await conn.execute<Array<{
      id: string;
      slug: string;
      local_path: string | null;
      remote_url: string | null;
      ...
      `SELECT r.id,
        ...
        ORDER BY r.last_touched_at DESC, r.id ASC`,
      [ctx.orgId],
    );

  return await Promise.all(repos.map(async (repo) => {
    const taskCountRows = await conn.execute<Array<{ count: number | string }>>(
    ...
    const lastCommitRows = await conn.execute<Array<{ message: string | null }>>(
      `SELECT message
         FROM repo_commits
        WHERE org_id = $1
          AND repo_id = $2
    ...
    const lastSyncAt = isoStamp(repo.last_sync_at);
    return {
      id: repo.id,
      slug: repo.slug,
      path: repo.local_path ?? repo.remote_url ?? repo.name ?? repo.slug,
      remoteUrl: repo.remote_url,
      branch: repo.current_branch ?? repo.default_branch,
      dirty: false,
      lastSyncAt,
      recentCommit: subject(lastCommitRows[0]?.message ?? null),
      openTaskCount: Number(taskCountRows[0]?.count ?? 0),
      health: repo.sync_status === "error" ? "failed" : lastSyncAt ? "healthy" : "stale",
...
export async function isRepoWriteOpsEnabled(em: EntityManager, ctx: AppContext): Promise<boolean> {
  const rows = await ormSqlConnection(em).execute<Array<{ enabled: boolean | null }>>(
    `SELECT enabled
       FROM feature_flags
...
export async function getRepoBranchesPage(em: EntityManager, ctx: AppContext, repoId: string): Promise<RepoBranchPageData> {
  ...
  const branches = await em.find(RepoBranch, { where: { org: { id: ctx.orgId }, repo: repoId } as never, order: { name: "ASC" } });
  return {
    repo: {
      id: repo.id,
...
export async function getRepoCommitDetail(
  ...
  const repo = await getRepo(em, ctx, input.repoId);
  const commit = await em.findOne(RepoCommit, { where: { org: { id: ctx.orgId }, repo: { id: input.repoId }, sha: input.sha } as never });
