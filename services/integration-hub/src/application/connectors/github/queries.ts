...
import type { AppContext, GithubConnectorStateDto } from "@integration-hub/application/connectors/github/types.ts";
...
export async function listGithubConnectorStates(em: EntityManager, ctx: AppContext, input: { repoFullName?: string } = {}): Promise<GithubConnectorStateDto[]> {
  const rows = await em.find(GithubConnectorState, { where: { org: { id: ctx.orgId }, ...(input.repoFullName ? { repoFullName: input.repoFullName } : {}) } as never, order: { updatedAt: "DESC", id: "ASC" } });
...
export async function getGithubConnectorState(em: EntityManager, ctx: AppContext, id: string): Promise<GithubConnectorStateDto> {
  const row = await em.findOne(GithubConnectorState, { where: { id } as never });
