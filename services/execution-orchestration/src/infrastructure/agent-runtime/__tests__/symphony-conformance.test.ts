...
const CONFORMANCE_TEST_PATH =
  "services/execution-orchestration/src/infrastructure/agent-runtime/__tests__/symphony-conformance.test.ts";
...
Prompt`);
  ...
  describe("HTTP extension (SYM-25)", () => {
    test("REQUIRED: http-api module binds loopback by default (127.0.0.1)", async () => {
      ...
      const httpServerPath = join(process.cwd(), "services/execution-orchestration/src/infrastructure/agent-runtime/symphony/http-server.ts");
      const httpApiPath = join(process.cwd(), "tests/support/product-workspace-fixtures.ts");
    ...
    test("REQUIRED: createHttpServer function exported from http-server module", async () => {
      // Either services/execution-orchestration/src/infrastructure/agent-runtime/symphony/http-server.ts or converged http-api.ts
      ...
      if (existsSync(httpServerPath)) {
        ...
        const mod = await import("@test-support/product-workspace-fixtures.ts");
    ...
    test("REQUIRED: GET /api/v1/state response includes generated_at, counts, running, retrying", async () => {
      const { createHttpApiRoutes } = await import("@test-support/product-workspace-fixtures.ts");
  ...
  describe("Dispatch parity — tRPC dispatchRun (SND-06)", () => {
    ...
    test("REQUIRED: dispatchRun output shape includes runId, state, agent, sandboxMode", async () => {
      const { createTestOrm } = await import("@test-support/application-database.ts");
