...
- `Agent` interface — `id`, `name`, `rootDir`, `rulesFile`, `skillsDir`, `mcpConfig`, etc. See `services/execution-orchestration/src/application/agent-catalog/registry.ts` for the live shape.
...
- `services/platform-core/src/application/component-lifecycle/adapters/*` — per-agent action execution.

## ADRs

Context-scoped decisions will live under `services/execution-orchestration/src/application/agent-catalog/docs/adr/` when recorded. None recorded yet; create the directory lazily from `docs/adr/0000-template.md`.
