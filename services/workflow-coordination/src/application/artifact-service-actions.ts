...
export async function listArtifacts(
  db: DbHandle,
  orgId: string,
  filter?: ArtifactFilter & { includeArchived?: boolean },): Promise<ArtifactRow[]> {
  if (isSqlExecutor(db)) {
  ...
  const rows = await conn.execute(
    `SELECT id, org_id, project_id, run_id, task_id, kind, title,
      ...
      WHERE ${where}
      ORDER BY created_at DESC, id ASC`,
    params,) as Array<ArtifactRow & { created_at: string | Date }>;
...
export async function readArtifactDetail(
  db: DbHandle,
  input: { orgId: string; id: string },): Promise<ArtifactDetail | null> {
  if (isSqlExecutor(db)) {
    const rows = await db.query<ArtifactRow & { created_at: string | Date }>(
      `SELECT id, org_id, project_id, run_id, task_id, kind, title,
              body_path, sha256, size, mime, COALESCE(archived, false) AS archived, created_at
...
export async function getArtifactStats(
  db: DbHandle,
  orgId: string,
  projectId: string,): Promise<ArtifactStats> {
  if (isSqlExecutor(db)) {
