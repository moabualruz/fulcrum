 ...
 *
 * Constructor accepts FulcrumCasbinAdapter and is injectable through needle-di.
 *
...
// ─────────────────────────────────────────────────────────────────────────────
// checkCasbinGate — utility consumed by permission middleware
// ─────────────────────────────────────────────────────────────────────────────

/**
 * checkCasbinGate — evaluates casbin enforcement for an application request.
 *
 * Logic:
 ...
 * @throws AppForbiddenError when Casbin explicitly denies.
 */
export async function checkCasbinGate(
  ...
  if (hasRule) {
    ...
    throw new AppForbiddenError(
      `Access denied: user '${userId}' does not have permission to '${action}' on '${resource}' in org '${orgId}'.`,
    );
  }
