...
import { createTask, setParent } from "@work-management/application/tasks/commands.ts";
import * as taskDetail from "@work-management/application/tasks/task-detail.ts";
...
describe("Workflow work item model and relationship hub", () => {
  ...
  test("task detail loads current edge schema, child rows, events, and bulk mutations", async () => {
    ...
    const events = await testDb.pglite.query<{ verb: string }>(
