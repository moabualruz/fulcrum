/**
 * WorkflowRulesService unit tests.
 *
...
describe("WorkflowRulesService", () => {
  describe("getDefaultWorkflow", () => {
    it("scrum returns 6-status flow with Todo and InReview", () => {
      const svc = new WorkflowRulesService(makeMockEm());
  ...
  describe("validateTransition", () => {
    ...
    it("throws NOT_FOUND when project does not exist", async () => {
      ...
      const svc = new WorkflowRulesService(em);
      await expect(svc.validateTransition("org-1", "proj-1", "Backlog", "Done"))
  ...
  describe("getEnabledTaskTypes / updateEnabledTaskTypes", () => {
    ...
    it("rejects invalid task types", async () => {
      const em = makeMockEm(makeProject());
      const svc = new WorkflowRulesService(em);
      await expect(svc.updateEnabledTaskTypes("org-1", "proj-1", ["epic", "invalid_type"]))
