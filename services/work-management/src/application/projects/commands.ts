...
export async function deleteProject(em: EntityManager, ctx: AppContext, id: string): Promise<{ ok: true }> {
  ...
  const deleted = await conn.execute<Array<{ id: string }>>(
...
export async function updateProjectTask(
  ...
  const sets: string[] = [];
  const params: unknown[] = [];
  const previousStatus = patch.status !== undefined
    ? await loadTaskStatusForUpdate(em, ctx, taskId)
    : null;
  ...
  if (previousStatus && patch.status !== undefined && patch.status !== null && previousStatus.status !== patch.status) {
    await appendEventOrm(em, {
      orgId: ctx.orgId,
      projectId: previousStatus.projectId ?? ctx.projectId ?? null,
      actor: ctx.userId ?? "system",
      subjectKind: "task",
      subjectId: taskId,
      verb: "status_changed",
      payload: { from: previousStatus.status, to: patch.status, task: taskId },
    });
  }
  return { ok: true };
}

async function loadTaskStatusForUpdate(
  em: EntityManager,
  ctx: AppContext,
  taskId: string,
): Promise<{ status: string; projectId: string | null } | null> {
  const rows = await ormSqlConnection(em).execute<Array<{ status: string; project_id: string | null }>>(
    `SELECT status, project_id FROM tasks WHERE id = $1 AND org_id = $2`,
    [taskId, ctx.orgId],
  );
  const row = rows[0];
  return row ? { status: row.status, projectId: row.project_id ?? null } : null;
}
