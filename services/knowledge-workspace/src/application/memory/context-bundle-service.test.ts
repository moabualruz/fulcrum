...
describe("ContextBundleService", () => {
  ...
  test("repoState returns empty array until repository state capture is wired", async () => {
    const bundle = await svc.assemble({ orgId: "org-1", projectId: "p1" });
    expect(bundle.repoState).toEqual([]);
  });

  test("skillPrompts returns empty array until skill prompt selection is wired", async () => {
    const bundle = await svc.assemble({ orgId: "org-1", projectId: "p1" });
