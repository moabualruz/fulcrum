/**
 * Memory actions — migrated from raw LegacyDatabaseHandle to TypeORM EntityManager.
 * ARCH-01/ARCH-02: All DB access via TypeORM connection.
 */

import type { EntityManager } from "typeorm";
import { randomUUID } from "node:crypto";
import { appendEventOrm, indexSearchDocumentOrm, ormSqlConnection } from "@platform-core/application/orm-helpers.ts";
...
export async function createMemoryAction(
  ...
  await appendEventOrm(em, { ...ctx, actor: "system", verb: "created", payload: { key: input.key, scope: input.scope } });
