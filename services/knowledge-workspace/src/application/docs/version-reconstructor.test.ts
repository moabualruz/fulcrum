...
describe("applyDelta", () => {
  ...
  it("Test 4: applyDelta with null delta throws an application invariant error", () => {
    expect(() => applyDelta(baseDoc, null, schema)).toThrow(AppInvariantError);
  });

  it("Test 5: applyDelta with empty steps array throws an application invariant error", () => {
    const delta = { steps: [] };
    expect(() => applyDelta(baseDoc, delta, schema)).toThrow(AppInvariantError);
  });
});
