...
- Central rule index installed once under `~/.fulcrum/rules/`, with per-agent loader blocks for cross-agent rules distribution.
...
`fulcrum install` copies `rules/` once to `~/.fulcrum/rules/`, then installs a small loader block in each detected agent's primary rules file. User content outside `<!-- BEGIN/END FULCRUM RULES -->` markers is preserved verbatim. Re-running install replaces only that loader block.

| Agent | Primary rules file | Method |
|---|---|---|
| Claude Code | `~/.claude/CLAUDE.md` | native `@` import to `~/.fulcrum/rules/AGENTS.md` |
| Gemini CLI | `~/.gemini/GEMINI.md` | native `@` import to `~/.fulcrum/rules/AGENTS.md` |
| Codex CLI | `~/.codex/AGENTS.md` | forced-read loader pointing at `~/.fulcrum/rules/AGENTS.md` |
| OpenCode | `~/.config/opencode/AGENTS.md` | forced-read loader pointing at `~/.fulcrum/rules/AGENTS.md` |
| Pi CLI | `~/.pi/agent/AGENTS.md` | forced-read loader pointing at `~/.fulcrum/rules/AGENTS.md` |

Project-level enforcement: keep project-specific behavior in the consumer repo's `AGENTS.md` or `GEMINI.md`. Put reusable Fulcrum behavior in `rules/AGENTS.md` and linked files under `rules/situational/`.
...
Fulcrum project instructions must preserve context-mode routing wherever Codex reads project rules. The root index is `rules/AGENTS.md`; detailed triggers live under `rules/situational/`.
...
- **`@fulcrum/ui-kit` is the only UI primitive source.** Every web/desktop surface composes existing ui-kit primitives - never re-implement a Button, Input, Select, Dialog, Toast, Table row, ModeRow, StatusBadge, etc. inline. When a PRD needs a primitive that does not exist yet, or an existing primitive lacks a required variant/prop/state, the same PRD must (1) add or extend the primitive under `packages/ui-kit/src/components/<name>/`, (2) export it from the barrel `packages/ui-kit/src/index.ts`, (3) add or extend its `/design-kit` fixture section and design-e2e coverage, (4) only then consume it from the feature route. Forbidden: route-local Svelte components that duplicate a ui-kit primitive's responsibility, hand-rolled overlays/dialogs/menus outside ui-kit, ad-hoc Tailwind utility clusters that re-create an existing variant. ui-kit primitives must stay OKLCH-tokened (no `#fff`/`#000`/raw hex/hsl in component CSS), expose stable `data-slot` hooks for design-e2e, ship ARIA roles/labels, and declare a `data-*` attribute for any controllable state (selection, expansion, tone, status). Variants are added via `tailwind-variants` or props - never by forking a component. The `ui_kit_adds:` field on a PRD must list every new component name and is serialized across concurrent PRDs.
...
- **Three top-level code roots - exact ownership.** Every code file lives under exactly one of three roots, by responsibility:
  - **`apps/**`** - runnable surfaces/processes: `web`, `cli`, `tui`, `server`, `desktop`, `daemon`. An app is an invocation/visualization layer: it composes services + packages, owns no business logic or persistence, and has a runnable entrypoint.
  - **`services/**`** - bounded DDD domain services (the 12 in `CONTEXT-MAP.md`). A service owns its domain logic, entities, repositories, migrations, and `domain/application/infrastructure/interface` layers. It is consumed by apps, never the reverse.
  - **`packages/**`** - cross-cutting shared libraries with **no domain ownership and no runnable entrypoint**: reusable primitives consumed by 2+ apps/services. A package is justified only when ≥2 consumers need it; a single-consumer "shared" lib belongs in that consumer.
- **`packages/` is for genuine cross-cutting shared code only - every package must have ≥2 real importers.** A `packages/<name>` with zero importers is dead scaffolding: either wire + adopt it (migrate the duplicated code in) or delete it - never leave it orphaned. Each package: `package.json` with `@fulcrum/<name>` name, a `src/` with an `index.ts` barrel, a tsconfig path alias `@fulcrum/<name>` in root `tsconfig.json` AND each consuming app's tsconfig, and a `CONTEXT.md`. Current canonical packages:
  - **`@fulcrum/ui-kit`** - the only UI primitive source (see the ui-kit-first rule). Consumed by `apps/web` (and `desktop`).
  - **`@fulcrum/shared-dto`** - cross-surface DTO/value types shared by ≥2 surfaces that are NOT owned by a single bounded service: workflow-stage/run/trace/status vocabulary used by web + cli + tui. Service-internal domain types stay in that service's `domain/`; only genuinely cross-surface contract types live here.
  - **`@fulcrum/test-fixtures`** - cross-service test factories + fixtures (fishery-style). Consumed by `tests/**` and service `*.test.ts`. Per-service test helpers stay with that service; only cross-service factories live here.
  - A new shared concern becomes a package only when a second consumer appears. Until then it stays with its single owner. When promoting: create the package, move the code, wire the alias in root + consumer tsconfigs, update `CONTEXT-MAP.md` §Packages, delete the old copies in the same commit.
- **One canonical home per shared concern - no duplication.** A type, util, or constant used by 2+ roots has exactly one definition, in the package that owns it. Re-defining `WorkflowStage`, `cn()`, trace/run/span id types, or any shared contract inline in an app/service is forbidden - import from `@fulcrum/shared-dto` / `@fulcrum/ui-kit`. Duplication discovered during any task is fixed in that task (consolidate to the package), not deferred.
...
- **CONTEXT files stay current.** Every `services/<name>/` and `apps/<name>/` owns a top-level `CONTEXT.md` following the [grill-with-docs CONTEXT format](./docs/agents/domain.md) (Language / Relationships / Example dialogue / Flagged ambiguities). Sub-areas under `src/application/<area>/` may add their own nested `CONTEXT.md` to sharpen parent vocabulary - do not promote area terms upward unless they're used outside the area. **[`CONTEXT-MAP.md`](./CONTEXT-MAP.md) is the index of all bounded contexts and surfaces; every new service/app, every rename, and every deletion updates it in the same commit.** Stale entries in the map (path no longer resolves) are correctness bugs - fix before merging unrelated work in that area. Domain language drifts: when introducing a term not already in the relevant `CONTEXT.md`, add it then, not later. When two terms refer to the same concept, flag the resolution under that file's "Flagged ambiguities" section. ADR conflicts surface explicitly per `docs/agents/domain.md`.
...
- `rules/AGENTS.md` - installed rule index. Different audience from this file: that = "how agent should behave inside any project", this = "what fulcrum is + where going".
