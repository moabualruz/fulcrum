...
Plugin / extension / package ownership wins over individual surfaces. Vendor package may carry MCP servers, skills, commands, rules, hooks, scripts, agents, mixed assets. Fulcrum installs/uninstalls package as vendor documents. Agents with the vendor's native package primitive use that primitive. Agents without it get nearest-native adapters with no behavior rewrite: package payload mirrors, loadable skill mirrors where the package cache is not a skill loader, and native MCP config generated from package `.mcp.json` / `mcp.json`. Package MCPs follow package install defaults and are enabled when the package is installed; builtin MCPs remain governed by the minimal/default-disabled policy above. Current examples: Repomix Claude plugins own Claude MCP/commands/agent and Gemini extension mirror owns Gemini MCP; Cloudflare Claude plugin owns bundled Claude MCP+skills while non-Claude agents get full mirrors, loadable skills, Cloudflare package MCP config, and cleanup of older standalone Cloudflare upstream skill copies; Superpowers native packages own Claude/Gemini/OpenCode/Pi, Codex gets full skill mirror.
