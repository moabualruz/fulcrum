# @fulcrum/memory Implementation Plan
...
**Architecture:** Extends @fulcrum/core's basic writeMemory/recallMemory with richer inputs and a proper recall strategy. Scoring is computed dynamically at recall time (never stored). Deduplication uses content hashing. Ingestion uses regex-based syntax chunking (no tree-sitter required).
...
```
packages/memory/
  package.json                        — name: @fulcrum/memory, dep: @fulcrum/core
...
**Key dependency from @fulcrum/core that MUST exist before this package:**
...
Expected: `@fulcrum/memory` appears in workspace. No errors.
...
git commit -m "chore(memory): scaffold @fulcrum/memory package"
```
...
Add MIGRATION_005 SQL and wire it into `runMigrations()`. This must be done in `@fulcrum/core`, not in `@fulcrum/memory`, so that all packages sharing the DB see the tables.
...
import { getDb } from '@fulcrum/core/src/db/client.js'
...
import { getDb } from '@fulcrum/core/src/db/client.js'
import { FulcrumError } from '@fulcrum/core/src/types.js'
import { contentHash, isDuplicate } from './dedup.js'
...
import { getDb } from '@fulcrum/core/src/db/client.js'
import { FulcrumError } from '@fulcrum/core/src/types.js'
import { getTextEmbedder } from '@fulcrum/core/src/embedding/registry.js'
...
function ftsSearch(
  db: ReturnType<typeof import('@fulcrum/core/src/db/client.js').getDb>,
  query: string,
  whereClause: string,
  whereParams: unknown[],
  limit: number
...
function updateAccessCounts(db: ReturnType<typeof import('@fulcrum/core/src/db/client.js').getDb>, ids: string[]): void {
...
| 12 | Finalize exports | `src/index.ts` | all public API accessible from `@fulcrum/memory` |
