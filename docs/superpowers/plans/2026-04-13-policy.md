# @moabualruz/fulcrum-policy Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the `@moabualruz/fulcrum-policy` package — rule-based policy engine with 6-scope hierarchy, 8 matcher types, 3 hardcoded SYSTEM_INVARIANTS, SecretGuard with 5 detection patterns, and an append-only audit log.

**Architecture:** New `packages/policy/` package depends on `@moabualruz/fulcrum-core` for DB access (`getDb()`/`setDb()`), `FulcrumError`, and `emitEvent()`. MIGRATION_004 (policy_rules + policy_events tables) is added to `runMigrations()` in `@moabualruz/fulcrum-core`. The engine evaluates SYSTEM_INVARIANTS (hardcoded, priority 1000) first, then loads workspace/project rules sorted by priority DESC, returns the first match, and defaults to allow. SecretGuard is a pure-sync function with no DB dependency — fully testable in isolation.
...
```
packages/policy/
  package.json                        — name: @moabualruz/fulcrum-policy, dep: @moabualruz/fulcrum-core
...
```json
{
  "name": "@moabualruz/fulcrum-policy",
  "version": "0.0.1",
  "type": "module",
  "main": "./src/index.ts",
  "types": "./src/index.ts",
...
Expected: `@moabualruz/fulcrum-policy` appears in workspace. No errors.
...
git commit -m "chore(policy): scaffold @moabualruz/fulcrum-policy package"
```
...
The policy_rules and policy_events tables must be created by `runMigrations()` in `@moabualruz/fulcrum-core`. Add MIGRATION_004 to `packages/core/src/db/migrations.ts`.
...
```bash
pnpm --filter @moabualruz/fulcrum-policy test -- secret-guard
```
...
```bash
pnpm --filter @moabualruz/fulcrum-policy test -- engine
```
...
```bash
pnpm --filter @moabualruz/fulcrum-policy test -- audit
```
...
```bash
pnpm --filter @moabualruz/fulcrum-policy test -- integration
```
...
git commit -m "feat(policy): @moabualruz/fulcrum-policy complete — engine, SecretGuard, audit, SYSTEM_INVARIANTS"
```
