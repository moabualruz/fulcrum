# @moabualruz/fulcrum-monitor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the `@moabualruz/fulcrum-monitor` package — analytics tables (daily, cycle, project, agent, team), metric query functions, burndown chart data, run replay, and a lightweight Hono HTTP server with SSE event streaming bound to `127.0.0.1`.

**Architecture:** New `packages/monitor/` package depends on `@moabualruz/fulcrum-core` for DB access (`getDb()`/`setDb()`). All analytics tables are created in MIGRATION_009, added to `runMigrations()` in `@moabualruz/fulcrum-core`. The HTTP server uses Hono + `@hono/node-server` and is always bound to `127.0.0.1` — never `0.0.0.0`. SSE polls the `events` table every 2 s. Tests use an in-memory SQLite database via `setDb(db)`.
...
```
packages/monitor/
  package.json                    — name: @moabualruz/fulcrum-monitor, deps: @moabualruz/fulcrum-core, hono, @hono/node-server
...
```json
{
  "name": "@moabualruz/fulcrum-monitor",
  "version": "0.0.1",
  "type": "module",
  "main": "./src/index.ts",
  "types": "./src/index.ts",
...
Expected: `@moabualruz/fulcrum-monitor` appears in workspace. `hono` and `@hono/node-server` resolved. No errors.
...
git commit -m "chore(monitor): scaffold @moabualruz/fulcrum-monitor package"
```
...
> **Note:** `runMigration009(db)` must also be called inside `runMigrations()` in `packages/core/src/db/migrations.ts`. Add a call after the existing highest-numbered migration. Import from `@moabualruz/fulcrum-monitor`.
...
Expected: all packages pass. `@moabualruz/fulcrum-monitor` shows 5 passing tests (or 6 with `FULCRUM_SERVER_TESTS=1`).
