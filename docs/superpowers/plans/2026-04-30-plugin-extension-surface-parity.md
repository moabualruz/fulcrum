...
> For agentic workers: REQUIRED SUB-SKILLS: use `subagent-orchestration`, `subagent-driven-development`, `test-driven-development`, and `verification-before-completion`. This plan is written for one continuous implementation run with maximum useful parallelism. Use checkbox syntax for progress.
