# @moabualruz/fulcrum-sync Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the `@moabualruz/fulcrum-sync` package — a 3-layer adapter-based synchronisation engine that pushes and pulls Fulcrum objects to/from external project-management tools (Plane.so first), with queue-based offline support, SHA-256 hash idempotency, conflict detection, and secret-redaction before transmission.

**Architecture:** `PlaneAPIClient` (Layer 1) owns raw HTTP calls to the Plane REST API; `PlaneSyncAdapter` (Layer 2) maps Fulcrum domain fields to/from Plane payloads and implements the `SyncAdapter` interface; `SyncManager` (Layer 3) owns queue processing, canonical hashing, conflict recording, and batch `syncAll`. The five public functions in `sync.ts` delegate to `SyncManager` and call `checkSecrets()` from `@moabualruz/fulcrum-policy` before any outbound push. MIGRATION_008 adds `sync_states`, `sync_conflicts`, and `sync_queue` tables to the shared SQLite database.
...
```
packages/sync/
  package.json                     — name: @moabualruz/fulcrum-sync, deps: @moabualruz/fulcrum-core, @moabualruz/fulcrum-policy
...
```json
{
  ...
  "dependencies": {
    ...
    "@moabualruz/fulcrum-policy": "workspace:*",
    "ulidx": "^2.3.0"
  },
  "devDependencies": {
...
Expected: `@moabualruz/fulcrum-sync` appears in workspace. No errors.

- [ ] **Step 6: Commit**

```bash
git add packages/sync/package.json packages/sync/tsconfig.json packages/sync/vitest.config.ts packages/sync/src/index.ts
git commit -m "chore(sync): scaffold @moabualruz/fulcrum-sync package"
```
...
```bash
cd /home/mkh/workspace/pi-stack-plan
pnpm --filter @moabualruz/fulcrum-sync test
```
...
## Task 9: Wire MIGRATION_008 into @moabualruz/fulcrum-core

**Context:** `runMigrations()` in `@moabualruz/fulcrum-core` must call `runMigration008` so the
...
```typescript
import { runMigration008 } from '@moabualruz/fulcrum-sync'

// Inside runMigrations(), after the last existing migration call:
runMigration008(db)
```

> **Important:** Only add this import if `@moabualruz/fulcrum-sync` is already listed as a
> dependency of `@moabualruz/fulcrum-core`. If circular dependency risk exists, call
...
- [ ] **Step 2: Add @moabualruz/fulcrum-sync as dependency of @moabualruz/fulcrum-core (if wired via core)**

```bash
cd /home/mkh/workspace/pi-stack-plan
pnpm --filter @moabualruz/fulcrum-core add @moabualruz/fulcrum-sync@'workspace:*'
```
...
**Secret redaction:** `checkSecrets()` from `@moabualruz/fulcrum-policy` is called in `syncObject` before any outbound adapter push. A match throws immediately — no data leaves the process.
