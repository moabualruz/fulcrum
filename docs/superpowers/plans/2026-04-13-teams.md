...
Expected: `@fulcrum/teams` appears in workspace. No errors.
...
git commit -m "chore(teams): scaffold @fulcrum/teams package"
```
...
```bash
pnpm --filter @fulcrum/teams test
```
...
feat(teams): implement @fulcrum/teams — TeamTemplate CRUD, TeamInstance lifecycle, slot occupancy
...
## Task 6: Integration with @fulcrum/core migrations

The teams migration must run when @fulcrum/core's `runMigrations()` is called, so consumers get the tables automatically. Add the teams SQL to `packages/core/src/db/migrations.ts`.
