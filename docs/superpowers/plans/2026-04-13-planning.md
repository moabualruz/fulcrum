# fulcrum-planning Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the `fulcrum-planning` package — Epic, Issue, PRD, Plan CRUD with optimistic locking, display IDs, status categories, sub-issue support, task relations, FTS5 triggers, and event emission.

**Architecture:** New `packages/planning/` package depends on `fulcrum-core` for DB access (`getDb()`/`setDb()`), `FulcrumError`, `emitEvent()`, `nextDisplayId()`, and `statusCategory()`. All tables are created in MIGRATION_003 which runs inside `runMigrations()` in `fulcrum-core`. Each domain entity (epic, issue, PRD, plan, relations) lives in its own file. Tests use an in-memory SQLite database via `createTestDb()` from a local helpers file that mirrors the core pattern.
...
```
packages/planning/
  package.json                        — name: fulcrum-planning, dep: fulcrum-core
...
Expected: `fulcrum-planning` appears in workspace. No errors.
...
git commit -m "chore(planning): scaffold fulcrum-planning package"
```
...
> NOTE: This requires that `runMigrations` in `fulcrum-core` already includes MIGRATION_003 (the planning tables). If it doesn't yet, add MIGRATION_003 to `packages/core/src/db/migrations.ts` before running these tests. The planning tables SQL is in the spec under "MIGRATION_003 — planning".
...
```bash
pnpm --filter fulcrum-planning test
```
...
```bash
pnpm --filter fulcrum-planning test -- epics
```
...
```bash
pnpm --filter fulcrum-planning test -- issues
```
...
```bash
pnpm --filter fulcrum-planning test -- prds
```
...
```bash
pnpm --filter fulcrum-planning test -- plans
```
...
```bash
pnpm --filter fulcrum-planning test -- relations
```
...
The planning tables (epics, issues, prds, plans) have FTS5 virtual tables that must stay synchronized via INSERT/UPDATE/DELETE triggers. These are added to MIGRATION_003 in `fulcrum-core/src/db/migrations.ts`.
...
```bash
pnpm --filter fulcrum-planning test -- integration
```
...
git commit -m "feat(planning): fulcrum-planning complete — epics, issues, prds, plans, relations, FTS5"
```
