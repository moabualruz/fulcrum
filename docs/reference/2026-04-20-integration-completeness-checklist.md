...
- terminal exception — landed with a named external/manual/decision blocker
- ⬜ not started
...
| **PermissionRequest hook (write-class interceptor; all tool types)** | ✅ | `grep -c 'runCodexPermissionRequestHook' packages/cli/src/index.ts` = 4; all-tool approval interceptor (unlike Bash-only PreToolUse) with secret-scan, team-invoke guard, and Codex-specific Fulcrum-first search telemetry/nudge; emits deny responses only when blocking; 7 tests cover allow, deny, recall, and search-nudge paths | **PR 6 v3.3 unit 6.2 + 2026-04-21 sixth-pass reopen** |
...
| **Installers/generators consume fanout or canonical source per host** | ✅ | `rg 'parseCanonicalSource\|emit(Codex\|Opencode\|Cursor\|Windsurf\|Copilot)' agent-integration/install.ts`; `scripts/fanout-gemini-extension.ts` uses `emitGemini`; Claude installs canonical source skills directly; sixth/seventh-pass unit ledger records per-host installer artifacts as accepted or explicit terminal blockers. | **PR 13 + sixth/seventh-pass ledger** |
...
| Fulcrum-first bias wired for **every audited hook-capable agent** (Claude, opencode, Gemini, PI, Codex, Cursor, Windsurf, Copilot) | ✅ | Claude (PR 3), opencode (PR 4 c4), and Gemini (PR 7 cross-cut) wire through `runPreHook`; PI injects a `before_agent_start` system-prompt nudge; Codex uses the all-tool `PermissionRequest` path for search telemetry/nudge because Codex PreToolUse is Bash-only; Cursor/Windsurf/Copilot read/search/MCP/session paths are covered by the host-runtime verifier and sixth/seventh-pass ledger. | **PR 5-8 staggered + 2026-04-21 sixth/seventh-pass reopen** |
