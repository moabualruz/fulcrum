...
The JSON ledger currently contains 2,958 explicit unit rows:

| Unit class | Rows |
|---|---:|
| workspace projects | 17 |
| package source files | 289 |
| package test files | 327 |
...
| Full unit ledger | terminal-all-accepted | 2,958 unit rows: 2,958 accepted, 0 blocked, 0 open. |
| Package internals and exports | accepted | Package roots, 289 production source files, 327 test files, 37 package configs, 37 generated dist artifacts, 80 manifest scripts, and 941 exports have accepted rows. |
...
- `pnpm --dir scripts test -- surface-inventory` passed with 12 inventory tests
  ...
  hook config events, PI extension events/commands, opencode plugin hooks, and
   2,958 explicit terminal unit status rows.
