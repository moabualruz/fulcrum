...
All span APIs are re-exported from `fulcrum-agent-core`. They're cheap enough that the runner and worker call them on every operation without asking the user to opt in.

```typescript
import { startSpan, endSpan, getTrace } from 'fulcrum-agent-core'
...
| `janitor.cycle` | Janitor reaping cycle in `fulcrum-agent-core` | reaped run counts, duration |
...
```typescript
import { getTrace } from 'fulcrum-agent-core'
...
```typescript
import { startSpan, endSpan } from 'fulcrum-agent-core'
...
If you embed `fulcrum-agent-core` in a long-running process you wrote yourself, register the same handler:

```typescript
import { shutdownOtel } from 'fulcrum-agent-core'
