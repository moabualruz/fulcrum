...
`fulcrum-agent-core` is the foundation layer. Every other package depends on it.
...
```typescript
startAgentRun(input)      // Start a run with context_type (call checkPolicy first)
...
```typescript
import { getEventBus, resetEventBus } from 'fulcrum-agent-core'
...
The six hook interface types are exported from `fulcrum-agent-core` so non-CLI packages can reference the hook contract without depending on `fulcrum-agent-cli`:
...
The implementations (`normalizeHookEvent`, `runPreHook`, `runPostHook`) live in `fulcrum-agent-cli` because they depend on `fulcrum-policy` and `fulcrum-memory`. The types are in core so policy or monitoring code can check hook shapes without pulling in the CLI package.
...
```typescript
import { getDb, runMigrations, _configureDb, setDb } from 'fulcrum-agent-core'
...
```typescript
import { withTransaction } from 'fulcrum-agent-core'
...
```typescript
import { checkDbHealth } from 'fulcrum-agent-core'
...
```typescript
import { startJanitor, loadConfig } from 'fulcrum-agent-core'
...
```typescript
import { initEmbedding, loadConfig } from 'fulcrum-agent-core'
