...
6. **Control-plane features default ON post-rollout.** During PRs 0-8 the curator auto-trigger and consolidation pass were dormant by default — a safety net so in-flight sessions didn't start firing the curator mid-migration. After PR 9 the posture flipped: `FULCRUM_MEMORY_CURATE_AUTO` defaults on (disabled only by explicit opt-out values `0`/`false`/`off`/`no`), and `FULCRUM_MEMORY_CONSOLIDATE_SCHEDULE` defaults to `daily` (disabled by `never`/`off`/`0`/`false`/`no` or any unknown cadence string). Operators opt OUT rather than opt IN; "dormant" is no longer a permanent policy.
