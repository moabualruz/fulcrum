...
`fulcrum install` defaults to `profile.minimal` through the component lifecycle engine. `--profile full` runs the historical `profile.default`; `--profile rules-only` runs only `rules.global`; `--enable-all-mcps` switches the target to `profile.verify-all`. `--no-skills`, `--no-upstream-skills`, and `--no-default-mcps` become planner exclusions plus the existing recommended MCP default-state compatibility step.

The minimal profile covers rules, policy, MCP registry setup, and recommended MCP defaults. The full profile adds hooks, skills, Caveman, upstream skills, and vendor packages:
